...
class FragWallet extends Fragment {
  override def onCreateView(
      ...
      viewGroup: ViewGroup,
      bundle: Bundle
  ) = inf.inflate(R.layout.frag_view_pager_btc, viewGroup, false)
  override def onViewCreated(view: View, state: Bundle) =
    if (app.isAlive)
      worker =
        new FragWalletWorker(getActivity.asInstanceOf[WalletActivity], view)
...
class FragWalletWorker(val host: WalletActivity, frag: View)
    extends SearchBar
    with HumanTimeDisplay { me =>
  import host.{
    UITask,
    onButtonTap,
    showForm,
  ...
  val fiatDetails =
    frag.findViewById(R.id.fiatDetails).asInstanceOf[LinearLayout]

  val mainWrap = frag.findViewById(R.id.mainWrap).asInstanceOf[LinearLayout]
  val mnemonicWarn =
    frag.findViewById(R.id.mnemonicWarn).asInstanceOf[LinearLayout]
  val itemsList = frag.findViewById(R.id.itemsList).asInstanceOf[ListView]

  val allTxsWrapper =
    host.getLayoutInflater.inflate(R.layout.frag_toggler, null)
  val toggler =
    allTxsWrapper.findViewById(R.id.toggler).asInstanceOf[ImageButton]
  ...
  with PeerDisconnectedEventListener {
    def onPeerDisconnected(peer: Peer, leftPeers: Int) =
      if (leftPeers < 1) updTitleTask.run
    def onPeerConnected(peer: Peer, leftPeers: Int) =
      if (leftPeers == 1) updTitleTask.run
  }
  ...
  val txsListener = new TxTracker with TransactionConfidenceEventListener {
    ...
    def onCoinsSent(w: Wallet, txj: Transaction, a: Coin, b: Coin) =
      if (a isGreaterThan b) updBtcItems
    def onCoinsReceived(w: Wallet, txj: Transaction, a: Coin, b: Coin) =
      if (b isGreaterThan a) updBtcItems
  ...
  val loaderCallbacks = new LoaderCallbacks[Cursor] {
    def onCreateLoader(id: Int, bn: Bundle) =
      new ReactLoader[PaymentInfo](host) {
        val consume = (payments: PaymentInfoVec) =>
          runAnd(lnItems = payments map LNWrap)(updPaymentList.run)
        def getCursor =
          if (lastQuery.isEmpty) PaymentInfoWrap.byRecent
          else PaymentInfoWrap.byQuery(lastQuery)
  ...
  val updTitleTask = UITask {
    val suspended =
      ChannelManager.all.filter(_.isHosted).filterNot(isOperational)
    ...
    val btcFunds =
      if (btcTotalSum.amount < 1) btcEmpty else denom parsedWithSign btcTotalSum
    val lnFunds =
      if (lnTotalSum.amount < 1) lnEmpty else denom parsedWithSign lnTotalSum
  ...
  val chanListener = new ChannelListener {
    def informOfferClose(chan: Channel, message: String, natRes: Int) = UITask {
      val bld =
        baseBuilder(title = chan.data.announce.asString.html, body = message)
      def onAccepted(alert: AlertDialog) =
        rm(alert)(chan process ChannelManager.CMDLocalShutdown)
      if (errorLimit > 0)
        mkCheckFormNeutral(
          _.dismiss,
          none,
    ...
    override def onProcessSuccess = {
      ...
      case (chan: HostedChannel, _: HostedCommits, remoteError: wire.Error) =>
        informOfferClose(
          chan,
          ChanErrorCodes.translateTag(remoteError).getMessage,
          natRes = -1
        ).run

      // Let user manually choose if remotely proposed override should be applied if chan is suspended
      case (chan: HostedChannel, hc: HostedCommits, so: wire.StateOverride)
          if chan.state == SUSPENDED =>
      ...
      case (chan: NormalChannel, _: NormalData, cr: ChannelReestablish)
          if cr.myCurrentPerCommitmentPoint.isEmpty =>
        informOfferClose(
          chan,
          app
            .getString(err_ln_peer_incompatible)
            .format(chan.data.announce.alias),
          ln_chan_close
        ).run
    }

    override def onBecome = {
      case (_, _, fromState, CLOSING) if fromState != CLOSING =>
        updPaymentList.run
    ...
    override def onException = {
      ...
      case _ \ CMDAddImpossible(rd, ERR_REMOTE_AMOUNT_HIGH, _) =>
        sendToOtherChanOrFailAndInform(app getString err_ln_remote_fee_high, rd)
      case _ \ CMDAddImpossible(rd, ERR_LOCAL_AMOUNT_HIGH, _) =>
        sendToOtherChanOrFailAndInform(app getString err_ln_local_fee_high, rd)
      case _ \ CMDAddImpossible(rd, ERR_TOO_MANY_HTLC, _) =>
        sendToOtherChanOrFailAndInform(app getString err_ln_too_many_htlc, rd)

      case _ \ CMDAddImpossible(rd, ERR_REMOTE_AMOUNT_LOW, min) =>
        val minHuman =
          s"<strong>${denom parsedWithSign min.millisatoshi}</strong>"
      ...
      case chan \ internalException =>
        val bld =
          negTextBuilder(dialog_ok, UncaughtHandler toText internalException)
        UITask(
          host showForm bld
            .setCustomTitle(chan.data.announce.asString.html)
            .create
        ).run
    }
  }
    ...
    action match {
      case act: UrlAction =>
        mkCheckFormNeutral(
          ...
          none,
          _ => share(act.url),
          paymentActionPopup(act.finalMessage.html, act),
          dialog_open,
          dialog_cancel,
          dialog_share
        )
      case act: MessageAction =>
        showForm(
          paymentActionPopup(act.finalMessage.html, act)
            .setNegativeButton(dialog_ok, null)
            .create
        )
  ...
  def paymentActionPopup(
      customMessage: CharSequence,
      paymentAction: PaymentAction
  ) = {
    val titleOpt = paymentAction.domain.map(
      site => s"$actionTitle<br><br><strong>$site</strong>"
    )
    host
      .baseTextBuilder(customMessage)
      .setCustomTitle(titleOpt.getOrElse(actionTitle).html)
  }

  def aesActionPopup(info: PaymentInfo, aes: AESAction) =
    Try {
      val plaintext = Tools.bin2readable(
        AES
          .dec(
            data = aes.ciphertextBytes,
            key = info.paymentPreimage.toArray,
            initVector = aes.ivBytes
          )
          .toArray
      )
      val msg =
        if (plaintext.length > 36)
          s"${aes.finalMessage}<br><br><tt>$plaintext</tt><br>"
        else s"${aes.finalMessage}<br><br><tt><big>$plaintext</big></tt><br>"
      plaintext -> msg.html
    } match {
      case Success(secret \ text) =>
        mkCheckFormNeutral(
          _.dismiss,
          none,
          _ => share(secret),
          paymentActionPopup(text, aes),
          dialog_ok,
          -1,
          dialog_share
        )
      case _ =>
        showForm(
          paymentActionPopup(
            s"<br>${app getString ln_url_pay_decrypt_fail}".html,
            aes
          ).setNegativeButton(dialog_ok, null).create
        )
    }

  def proposeOverride(
      cmd: CMDHostedStateOverride,
      chan: HostedChannel,
      hc: HostedCommits
  ) = UITask {
    val currentBalance =
      s"<strong>${denom.parsedWithSign(chan.estCanSendMsat.toTruncatedMsat).html}</strong>"
    val proposedBalance =
      s"<strong>${denom.parsedWithSign(hc.newLocalBalanceMsat(cmd.so).toTruncatedMsat).html}</strong>"
    val hostedOverrideDetails = app
      .getString(ln_hosted_override_warn)
      .format(currentBalance, proposedBalance)
      .html
    val bld = host
      .baseTextBuilder(hostedOverrideDetails)
      .setCustomTitle(chan.data.announce.asString.html)
    mkCheckForm(
      alert => rm(alert)(chan process cmd),
      none,
      bld,
      dialog_ok,
      dialog_cancel
  ...
  def sendToOtherChanOrFailAndInform(message: CharSequence, rd: RoutingData) = {
    ChannelManager.sendEither(
      useFirstRoute(rd.routes.tail, rd),
      informNoRoutesLeft
    )
  ...
  def updPaymentList = UITask {
    ...
    val tempItemWraps =
      if (isSearching) lnItems else delayedWraps ++ btcItems ++ lnItems
    fundTxIds = ChannelManager.all.collect {
      case normalChannel: NormalChannel => normalChannel.fundTxId.toHex
    }.toSet
    sentHostedPreimages = ChannelManager.all
      .map(_.data)
      .collect { case hc: HostedCommits => hc.sentPreimages }
      .flatten
      .toMap
    allItems = tempItemWraps
      .sortBy(_.getDate)(Ordering[java.util.Date].reverse) take 48
  ...
  val adapter = new BaseAdapter {
    ...
    def getView(position: Int, savedView: View, parent: ViewGroup) = {
      val resource =
        if (isTablet) R.layout.frag_tx_line_tablet else R.layout.frag_tx_line
      val view =
        if (null == savedView) host.getLayoutInflater.inflate(resource, null)
        else savedView
      val holder =
        if (null == view.getTag) ViewHolder(view)
        else view.getTag.asInstanceOf[ViewHolder]
  ...
  case class ViewHolder(view: View) {
    val transactCircle =
      view.findViewById(R.id.transactCircle).asInstanceOf[ImageView]
    val transactWhen =
      view.findViewById(R.id.transactWhen).asInstanceOf[TextView]
    val transactWhat =
      view.findViewById(R.id.transactWhat).asInstanceOf[TextView]
  ...
  case class ShowDelayedWrap(stat: ShowDelayed) extends ItemWrap {
    ...
    def generatePopup = {
      val detailsWrapper =
        host.getLayoutInflater.inflate(R.layout.frag_tx_btc_details, null)
      val useExplorerButton =
        detailsWrapper.findViewById(R.id.viewTxOutside).asInstanceOf[Button]
      useExplorerButton setOnClickListener onButtonTap(
        host browse s"https://smartbit.com.au/tx/$txid"
      )
  ...
  case class LNWrap(info: PaymentInfo) extends ItemWrap {
    ...
    def hostedBlocks: Option[Long] =
      sentHostedPreimages.get(info.paymentPreimage)
    override def backgroundColor =
      if (hostedBlocks.isDefined) Denomination.yellowHighlight else 0x00000000
    ...
    def fillView(holder: ViewHolder) = {
      ...
      if (isTablet || isSearching)
        holder.transactWhat setText humanDesc(info.pd.text).html
    ...
    def generatePopup = {
      val humanStatus = info.status match {
        case FAILURE if 0 == info.incoming =>
          s"<strong>${app getString ln_state_fail_out}</strong>"
        case FAILURE if 1 == info.incoming =>
          s"<strong>${app getString ln_state_fail_in}</strong>"
      ...
      val detailsWrapper =
        host.getLayoutInflater.inflate(R.layout.frag_tx_ln_details, null)
      val paymentProof =
        detailsWrapper.findViewById(R.id.paymentProof).asInstanceOf[Button]
      val paymentDebug =
        detailsWrapper.findViewById(R.id.paymentDebug).asInstanceOf[Button]
      val paymentRequest =
        detailsWrapper.findViewById(R.id.paymentRequest).asInstanceOf[Button]
      val runPaymentAction =
        detailsWrapper.findViewById(R.id.runPaymentAction).asInstanceOf[Button]
      val paymentDetails =
        detailsWrapper.findViewById(R.id.paymentDetails).asInstanceOf[TextView]
      ...
      if (info.status == SUCCESS) {
        paymentProof setOnClickListener onButtonTap(
          host share app
            .getString(ln_proof)
            .format(serializedPR, info.preimage)
            .trim
        )
        for (act <- info.pd.action)
          runPaymentAction setOnClickListener onButtonTap(
            showPayAction(info, act).run
          )
      ...
      PaymentInfoWrap.acceptedPayments get info.pr.paymentHash foreach { rd1 =>
        val routingPath =
          for (usedHop <- rd1.usedRoute) yield usedHop.humanDetails
        val failures = PaymentInfo.errors(info.pr.paymentHash) map {
          case fail \ _ => fail.toString
        } mkString "\n==\n"
        val receiverInfo =
          s"Payee node ID: ${rd1.pr.nodeId.toString}, Expiry: ${rd1.pr.adjustedMinFinalCltvExpiry} blocks"
      ...
      def outgoingTitle = {
        ...
        val sentHuman =
          if (info.isLooper) denom.coloredP2WSH(info.firstSum, denom.sign)
          else denom.coloredOut(info.firstSum, denom.sign)
        app
          .getString(ln_outgoing_title)
          .format(
            humanStatus,
            sentHuman,
            inFiat,
            denom.coloredOut(fee, denom.sign),
            paidFeePercent
          )
      }
      ...
      info.incoming -> hostedBlocks match {
        case 0 \ _ if info.lastExpiry == 0 =>
          ...
          val title = app
            .getString(ln_outgoing_title_no_fee)
            .format(
              humanStatus,
              denom.coloredOut(info.firstSum, denom.sign),
              inFiat
            )
          showForm(negBuilder(dialog_ok, title.html, detailsWrapper).create)
        ...
        case 0 \ _ if info.status == FAILURE =>
          val newRD = app
            .emptyRD(info.pr, info.firstMsat, useCache = false)
            .copy(action = info.pd.action)
          // This payment has been tried and failed, allow to retry this one unless its payment request has expired already
          val show = mkCheckForm(
            _.dismiss,
            doSendOffChain(newRD),
            baseBuilder(outgoingTitle.html, detailsWrapper),
            dialog_ok,
            _: Int
          )
        ...
        case 0 \ _ =>
          ...
          val expiryBlocksLeft = app.plur1OrZero(
            lnStatusExpiry,
            info.lastExpiry - broadcaster.currentHeight
          )
          val title =
            if (info.status == WAITING)
              s"$expiryBlocksLeft<br><br>$outgoingTitle"
            else outgoingTitle
          showForm(negBuilder(dialog_ok, title.html, detailsWrapper).create)

        case 1 \ Some(blocksUntilExpiry)
            if info.isLooper && info.status == WAITING =>
          // This is a reflexive payment, we are receiving through a hosted channel and preimage is revealed
          val expiryBlocksLeft = app.plur1OrZero(
            lnStatusExpiry,
            blocksUntilExpiry - broadcaster.currentHeight
          )
          val title =
            s"$expiryBlocksLeft<br><br>${app getString ln_hosted_preimage_revealed}<br><br>$outgoingTitle"
          showForm(negBuilder(dialog_ok, title.html, detailsWrapper).create)
        ...
        case 1 \ Some(blocksUntilExpiry) if info.status == WAITING =>
          ...
          val incomingTitle = app
            .getString(ln_incoming_title)
            .format(
              humanStatus,
              denom.coloredIn(info.firstSum, denom.sign),
              inFiat
            )
          val title =
            s"$expiryBlocksLeft<br><br>${app getString ln_hosted_preimage_revealed}<br><br>$incomingTitle"
          showForm(negBuilder(dialog_ok, title.html, detailsWrapper).create)
        ...
        case 1 \ _ =>
          ...
          if (info.isLooper)
            showForm(
              alertDialog = negBuilder(
                dialog_ok,
                title = outgoingTitle.html,
                body = detailsWrapper
              ).create
            )
          else if (info.status != WAITING)
            showForm(
              negBuilder(dialog_ok, incomingTitle.html, detailsWrapper).create
            )
  ...
  case class BTCWrap(wrap: TxWrap) extends ItemWrap {
    ...
    def fillView(holder: ViewHolder) = {
      val humanAmount =
        if (fundTxIds contains txid)
          denom.coloredP2WSH(-wrap.visibleValue, new String)
        else if (wrap.visibleValue.isPositive)
          denom.coloredIn(wrap.visibleValue, new String)
        else denom.coloredOut(-wrap.visibleValue, new String)

      val status =
        if (txDead) dead else if (txDepth >= minDepth) conf1 else await
      holder.transactWhen setText when(System.currentTimeMillis, getDate).html
    ...
    def generatePopup = {
      val confs =
        if (txDead) txsConfs.last else app.plur1OrZero(txsConfs, txDepth)
      val detailsWrapper =
        host.getLayoutInflater.inflate(R.layout.frag_tx_btc_details, null)
      val humanValues = wrap.directedScriptPubKeysWithValueTry(
        wrap.visibleValue.isPositive
      ) collect {
        case Success(channelFunding \ value) if channelFunding.isSentToP2WSH =>
          P2WSHData(value, channelFunding)
        case Success(pks \ value) if !ScriptPattern.isOpReturn(pks) =>
          AddrData(value, pks getToAddress app.params)
      } collect {
        case contract: P2WSHData =>
          contract destination denom.coloredP2WSH(contract.cn, denom.sign)
        case incoming: AddrData if wrap.visibleValue.isPositive =>
          incoming destination denom.coloredIn(incoming.cn, denom.sign)
        case outgoingPayment: AddrData =>
          outgoingPayment destination denom.coloredOut(
            outgoingPayment.cn,
            denom.sign
          )
      }
      ...
      val views = new ArrayAdapter(
        host,
        R.layout.frag_top_tip,
        R.id.titleTip,
        humanValues.map(_.html).toArray
      )
      val lst = host.getLayoutInflater
        .inflate(R.layout.frag_center_list, null)
      ...
      val header = wrap.fee match {
        case _ if wrap.visibleValue.isPositive =>
          ...
          val receivedHumanAmount =
            denom.coloredIn(wrap.visibleValue, denom.sign)
          app
            .getString(btc_incoming_title)
            .format(confs, receivedHumanAmount, inFiat)
        ...
        case None =>
          ...
          app
            .getString(btc_outgoing_title_no_fee)
            .format(confs, humanAmount, inFiat)
      }

      // Check if CPFP can be applied: enough value to handle the fee, not dead yet
      if (wrap.valueDelta.isLessThan(RatesSaver.rates.feeSix) || txDepth > 0)
        showForm(negBuilder(dialog_ok, header.html, lst).create)
      else
        mkCheckForm(
          _.dismiss,
          boostIncoming(wrap),
          baseBuilder(header.html, lst),
          dialog_ok,
          dialog_boost
        )
    }
  ...
  def receive(
      chansWithRoutes: Map[Channel, PaymentRoute],
      maxCanReceive: MilliSatoshi,
      minCanReceive: MilliSatoshi,
      title: View,
      desc: String
  )(onDone: RoutingData => Unit) = {

    val baseHint = app
      .getString(amount_hint_can_receive)
      .format(denom parsedWithSign maxCanReceive)
    val content = host.getLayoutInflater.inflate(
      R.layout.frag_ln_input_receive,
      null,
      false
    )
    val inputDescription =
      content.findViewById(R.id.inputDescription).asInstanceOf[EditText]
    ...
    def makeNormalRequest(sum: MilliSatoshi) = {
      ...
      PaymentInfoWrap
        .recordRoutingDataWithPr(
          extraRoutes = mostViableChannels map chansWithRoutes,
          amount = sum,
          preimage = ByteVector.view(random getBytes 32),
          inputDescription.getText.toString.take(72).trim
        )
    }
  ...
  abstract class OffChainSender(
      val maxCanSend: MilliSatoshi,
      val minCanSend: MilliSatoshi
  ) {
    val baseContent = host.getLayoutInflater.inflate(
      R.layout.frag_input_fiat_converter,
      null,
      false
    )
    val baseHint = app
      .getString(amount_hint_can_send)
      .format(denom parsedWithSign maxCanSend)
  ...
  def standardOffChainSend(pr: PaymentRequest) =
    new OffChainSender(
      maxCanSend = ChannelManager.estimateAIRCanSend.millisatoshi,
      minCanSend = pr.msatOrMin
    ) {
      def displayPaymentForm =
        mkCheckForm(
          sendAttempt,
          none,
          baseBuilder(getTitle, baseContent),
          dialog_pay,
          dialog_cancel
        )
      def getTitle =
        str2View(
          app
            .getString(ln_send_title)
            .format(Utils humanDesc pr.description)
            .html
        )
      ...
      pr.fallbackAddress -> pr.amount match {
        case Some(adr) \ Some(amount)
            if amount > maxCanSend && amount < app.kit.conf0Balance =>
          def sendOnChain: Unit =
          ...
          mkCheckFormNeutral(
            _.dismiss,
            none,
            alert => rm(alert)(sendOnChain),
            baseBuilder(getTitle, failureMsg.html),
            dialog_ok,
            -1,
            dialog_pay_onchain
          )
  ...
  def lnurlPayOffChainSend(domain: String, payReq: PayRequest) = {
    new OffChainSender(
      maxCanSend = math
        .min(ChannelManager.estimateAIRCanSend, payReq.maxSendable)
        .millisatoshi,
      minCanSend = payReq.minSendable.millisatoshi
    ) {
      def displayPaymentForm =
        mkCheckFormNeutral(
          sendAttempt,
          none,
          _ => browse(s"https://$domain"),
          baseBuilder(getTitle, baseContent),
          dialog_ok,
          dialog_cancel,
          dialog_info
      ...
      def getTitle = {
        val content =
          s"<strong><big>$domain</big></strong><br><br>${payReq.metaDataTextPlain take 72}"
        host.updateView2Blue(
          oldView = str2View(new String),
          app.getString(ln_send_title) format content
        )
      }
      ...
      def onUserAcceptSend(ms: MilliSatoshi) = {
        ...
        def convert(raw: String) = {
          ...
          val descriptionHash =
            ByteVector.fromValidHex(prf.paymentRequest.description)
          require(
            descriptionHash == payReq.metaDataHash,
            s"Metadata hash mismatch, original=${payReq.metaDataHash}, provided=$descriptionHash"
          )
          require(
            PaymentRequest
              .prefixes(LNParams.chainHash) == prf.paymentRequest.prefix,
            s"Wrong network prefix=${prf.paymentRequest.prefix}"
          )
          require(
            prf.paymentRequest.amount.contains(ms),
            s"Payment amount mismatch, provided=${prf.paymentRequest.amount}, requested=$ms"
          )
          prf
            .modify(_.successAction.domain.each)
            .setTo(payReq.callbackUri.getHost)
        }
        ...
        def send(prf: PayRequestFinal) = {
          ...
          val rd1 = rd.copy(
            action = Some(prf.successAction),
            airLeft = ChannelManager.all count isOperational
          )
          UITask(me doSendOffChain rd1).run
        }

        queue
          .map(_ => payReq.requestFinal(ms).body)
  ...
  def doSendOffChain(rd: RoutingData): Unit = {
    ...
    val accumulatorChannel = ChannelManager.all
      ...
      .filter(
        chan =>
          chan.estCanReceiveMsat + chan.estCanSendMsat >= maxAcceptableFee(
            rd.firstMsat,
            hops = 3
          ) + rd.firstMsat
      )
      .sortBy(_.estCanReceiveMsat)
      .headOption // The one closest to needed amount, meaning as few/low AIR transfers as possible

    sendableDirectlyOrAlternatives -> accumulatorChannel match {
      case Left(_ \ SENDABLE_AIR) \ Some(accum) =>
        <(startAIR(accum, rd), onFail)(none)
      case Left(unsendableAirImpossible \ _) \ _ =>
        app quickToast unsendableAirImpossible
  ...
  def startAIR(toChan: Channel, origEmptyRD: RoutingData) = {
    ...
    val deltaAmountToSend = maxAcceptableFee(rd1.firstMsat, hops = 3) + rd1.firstMsat - math
      .max(toChan.estCanSendMsat, 0L)
    val amountPossibleRebalance = ChannelManager
      .airCanSendInto(targetChan = toChan)
      .reduceOption(_ max _) getOrElse 0L
    require(
      deltaAmountToSend > 0,
      "Accumulator already has enough money for a final payment + fees"
    )
    require(
      amountPossibleRebalance > 0,
      "No channel is able to send funds into accumulator"
    )

    val Some(_ \ extraHop) = channelAndHop(toChan)
    val rbRD = PaymentInfoWrap.recordRoutingDataWithPr(
      extraRoutes = Vector(extraHop),
      amount = math.min(deltaAmountToSend, amountPossibleRebalance).millisatoshi,
      preimage = ByteVector(random getBytes 32),
      REBALANCING
    )
    ...
    val listener = new ChannelListener {
      override def outPaymentAccepted(rd: RoutingData) =
        ...
        if (rd.pr.paymentHash != rbRD.pr.paymentHash)
          ChannelManager detachListener this

      override def onSettled(cs: Commitments) = {
        val isOK = cs.localSpec.fulfilledOutgoing
          .exists(_.paymentHash == rbRD.pr.paymentHash)
  ...
  def sendBtcPopup(uri: BitcoinURI): RateManager = {
    val minMsatAmountTry = Try(uri.getAmount)
      .filter(org.bitcoinj.core.Transaction.MIN_NONDUST_OUTPUT.isLessThan)
      .map(coin2MSat)
    val minMsatAmount = minMsatAmountTry getOrElse coin2MSat(
      org.bitcoinj.core.Transaction.MIN_NONDUST_OUTPUT
    )
    val baseHint = app
      .getString(amount_hint_can_send)
      .format(denom parsedWithSign app.kit.conf0Balance)
    val content = host.getLayoutInflater.inflate(
      R.layout.frag_input_fiat_converter,
      null,
      false
    ...
    def sendAttempt(alert: AlertDialog): Unit = rateManager.result match {
      case Success(small) if small < minMsatAmount =>
        app quickToast dialog_sum_small
      ...
      case Success(ms) =>
        val txProcessor = new TxProcessor {
          ...
          def futureProcess(unsignedRequest: SendRequest) =
            app.kit blockSend app.kit.sign(unsignedRequest).tx
          def onTxFail(err: Throwable): Unit =
            mkCheckForm(_.dismiss, none, txMakeErrorBuilder(err), dialog_ok, -1)
        }
    ...
    def useMax(alert: AlertDialog) =
      rateManager setSum Try(app.kit.conf0Balance)
    val title = app
      .getString(btc_send_title)
      .format(Utils humanSix uri.getAddress.toString)
      .html
    mkCheckFormNeutral(
      sendAttempt,
      none,
      useMax,
      baseBuilder(title, content),
      dialog_next,
      dialog_cancel,
      dialog_max
    )
  ...
  def boostIncoming(wrap: TxWrap) = {
    ...
    val userWarn = baseBuilder(
      app.getString(boost_details).format(current, boost).html,
      null
    )
    mkCheckForm(
      _.dismiss,
      <(replace, onError)(none),
      userWarn,
      dialog_cancel,
      dialog_boost
    )
  ...
  def react =
    android.support.v4.app.LoaderManager
      .getInstance(host)
      .restartLoader(1, null, loaderCallbacks)
      .forceLoad
  val observer = new ContentObserver(new Handler) {
    override def onChange(fromSelf: Boolean) = if (!fromSelf) react
  }
  host.getContentResolver.registerContentObserver(
    db sqlPath PaymentTable.table,
    true,
    observer
  )
  ...
  toggler setOnClickListener onButtonTap {
    val newImg =
      if (currentCut > minLinesNum) ic_explode_24dp else ic_implode_24dp
