...
object Utils {
  ...
  def humanSix(bitcoinAddress: String) =
    bitcoinAddress.grouped(size = 6).mkString(sep = "\u0020")
  def humanDesc(raw: String) =
    if (raw.isEmpty) s"<i>${app getString ln_no_description}</i>"
    else raw take 72
  ...
  def msatInFiat(milliSatoshi: MilliSatoshi) = currentRate map { perBtc =>
    milliSatoshi.amount * perBtc / BtcDenomination.factor
  }

  val msatInFiatHuman: MilliSatoshi => String = ms =>
    msatInFiat(ms) match {
      case Success(providedAmount) =>
        s"≈ ${formatFiat format providedAmount} $fiatCode"
      case _ => s"≈ ? $fiatCode"
    }
}
...
trait TimerActivity extends AppCompatActivity { me =>
  ...
  def baseBuilder(title: View, body: View) =
    new Builder(me).setCustomTitle(title).setView(body)
  def negTextBuilder(neg: Int, msg: CharSequence) =
    baseTextBuilder(msg).setNegativeButton(neg, null)
  def negBuilder(neg: Int, title: View, body: View) =
    baseBuilder(title, body).setNegativeButton(neg, null)
  ...
  def onFail(error: CharSequence): Unit =
    UITask(me showForm negBuilder(dialog_ok, null, error).create).run
  ...
  def mkCheckForm(
    ...
    if (-1 != noResource)
      alert getButton BUTTON_NEGATIVE setOnClickListener negAct
    if (-1 != okResource)
      alert getButton BUTTON_POSITIVE setOnClickListener posAct
    alert
  }
  ...
  def <[T](fun: => T, no: Throwable => Unit)(ok: T => Unit) =
    Future(fun) onComplete {
      case Success(rs) => UITask(ok apply rs).run
      case Failure(ex) => UITask(no apply ex).run
    }

  // Utils
  def onButtonTap(exec: => Unit) = new OnClickListener {
    def onClick(view: View) = exec
  }
  def onTap(listItemTapRunner: Int => Unit): OnItemClickListener =
    new OnItemClickListener {
      def onItemClick(a: AdapterView[_], view: View, pos: Int, id: Long) =
        listItemTapRunner(pos)
    }
  ...
  def viewMnemonic(view: View) = {
    val recoveryCode =
      TextUtils.join("\u0020", app.kit.wallet.getKeyChainSeed.getMnemonicCode)
    showForm(
      negBuilder(dialog_ok, me getString sets_mnemonic, recoveryCode).create
    )
  }
  ...
  abstract class TxProcessor { self =>
    ...
    def start(coloredAmount: String) =
      <(app.kit sign plainRequest(RatesSaver.rates.feeSix), onTxFail) {
        estimate =>
          ...
          val txtFeeLive =
            getString(fee_live).format(markedLivePerTxFee, inFiatLive)
          val txtFeeRisky =
            getString(fee_risky).format(markedRiskyPerTxFee, inFiatRisky)
          val form =
            getLayoutInflater.inflate(R.layout.frag_input_choose_fee, null)
          val lst = form.findViewById(R.id.choiceList).asInstanceOf[ListView]
          val feesOptions = Array(txtFeeRisky.html, txtFeeLive.html)

          def proceed =
            <(
              lst.getCheckedItemPosition match {
                // Allow user to choose an economical fee when sending a manual transaction
                case 0 =>
                  self futureProcess plainRequest(RatesSaver.rates.feeSix div 2)
                case 1 =>
                  self futureProcess plainRequest(RatesSaver.rates.feeSix)
              },
              onTxFail
            )(none)
          ...
          mkCheckForm(
            alert => rm(alert)(proceed),
            none,
            baseBuilder(stepFees, form),
            dialog_pay,
            dialog_cancel
          )
          lst setAdapter new ArrayAdapter(
    ...
    def txMakeErrorBuilder: PartialFunction[Throwable, Builder] = {
      case _: ExceededMaxTransactionSize =>
        baseBuilder(app getString err_tx_too_large, null)
      case _: CouldNotAdjustDownwards =>
        baseBuilder(app getString err_empty_shrunk, null)

      case notEnough: InsufficientMoneyException =>
        // Payment itself is fine but exceeds balance once miner fee is added
        val totalFee =
          app.kit.conf0Balance.minus(pay.cn).plus(notEnough.missing)

        val fee = s"<strong>${denom parsedWithSign totalFee}</strong>"
        val sending = s"<strong>${denom parsedWithSign pay.cn}</strong>"
        val canSend =
          s"<strong>${denom parsedWithSign app.kit.conf0Balance}</strong>"
        val msg =
          getString(err_not_enough_funds).format(canSend, sending, fee).html
        baseTextBuilder(msg)
...
class RateManager(val content: View) { me =>
  ...
  val fiatInput =
    content.findViewById(R.id.fiatInputAmount).asInstanceOf[EditText]
  val hintFiatDenom = Utils clickableTextField content.findViewById(
    R.id.hintFiatDenom
  )
  ...
  def setSum(res: TryMSat) =
    satInput.setText(res map denom.asString getOrElse null)
  def hint(ex: String) =
    runAnd(me)(hintDenom setText denom.amountInTxt.format(ex).html)

  val fiatListener = new TextChangedWatcher {
    def upd =
      currentRate.map(
        perBtc => BigDecimal(fiatInput.getText.toString.noSpaces) / perBtc
      ) map btcBigDecimal2MSat
    def onTextChanged(s: CharSequence, start: Int, before: Int, count: Int) =
      if (fiatInput.hasFocus) setSum(upd)
  }

  val bitListener = new TextChangedWatcher {
    def upd =
      fiatInput.setText(
        result flatMap msatInFiat map formatFiat.format getOrElse null
      )
    def onTextChanged(s: CharSequence, start: Int, before: Int, count: Int) =
      if (satInput.hasFocus) upd
  }
...
case class AddrData(cn: Coin, address: Address) extends PayData {
  def getRequest: SendRequest =
    if (app.kit.conf0Balance equals cn) emptyWallet(address)
    else to(address, cn)
  def destination(colored: String) =
    s"<small>$colored</small><br>" + humanSix(address.toString)
}

case class P2WSHData(cn: Coin, pay2wsh: Script) extends PayData {
  def getRequest =
    if (app.kit.conf0Balance equals cn) emptyWallet(app.params, pay2wsh)
    else to(app.params, pay2wsh, cn)
  def destination(colored: String) =
    s"<small>$colored</small><br>" + app.getString(txs_p2wsh)
}
