...
trait SearchBar { me =>
  ...
  def setupSearch(m: Menu) = {
    searchView =
      m.findItem(R.id.action_search).getActionView.asInstanceOf[SearchView]
    searchView addOnAttachStateChangeListener new View.OnAttachStateChangeListener {
      def onViewDetachedFromWindow(lens: View) =
        runAnd(isSearching = false)(react)
...
trait HumanTimeDisplay {
  ...
  def when(now: Long, thenDate: Date) = thenDate.getTime match {
    case ago =>
...
class WalletActivity extends NfcReaderActivity with ScanActivity { me =>
  lazy val floatingActionMenu = findViewById(R.id.fam)
    .asInstanceOf[FloatingActionMenu]
  lazy val slidingFragmentAdapter = new FragmentStatePagerAdapter(
    getSupportFragmentManager
  ) {
    def getItem(currentFragmentPos: Int) =
      if (0 == currentFragmentPos) new FragWallet else new FragScan
    def getCount = 2
  }
  ...
  override def onCreateOptionsMenu(menu: Menu) = {
    ...
    FragWallet.worker.searchView
      .setQueryHint(app getString search_hint_payments)
    val openAutoHostedChan =
      app.prefs.getBoolean(AbstractKit.AUTO_HOSTED_CHAN, false)
    val showTooltip = app.prefs.getBoolean(AbstractKit.SHOW_TOOLTIP, true)

    if (openAutoHostedChan) {
      var hasDefaultHosted = ChannelManager.hasHostedChanWith(
        FragLNStart.defaultHostedNode.ann.nodeId
      )
      if (hasDefaultHosted)
        app.prefs.edit.putBoolean(AbstractKit.AUTO_HOSTED_CHAN, false).commit
      else {
        val refundScriptPubKey: ByteVector = ByteVector(
          ScriptBuilder.createOutputScript(app.kit.currentAddress).getProgram
        )
        val waitData = WaitRemoteHostedReply(
          FragLNStart.defaultHostedNode.ann,
          refundScriptPubKey,
          FragLNStart.defaultHostedNode.secret
        )
        val freshChannel =
          ChannelManager.createHostedChannel(Set.empty, waitData)

        lazy val hostedChanOpenListener = new ConnectionListener
        with ChannelListener {
          override def onDisconnect(nodeId: PublicKey) =
            if (nodeId == FragLNStart.defaultHostedNode.ann.nodeId)
              detachAll(retryOnRestart = true)
          override def onOperational(nodeId: PublicKey, isCompat: Boolean) =
            if (nodeId == FragLNStart.defaultHostedNode.ann.nodeId && isCompat)
              freshChannel.startUp
          override def onHostedMessage(
              ann: NodeAnnouncement,
              message: HostedChannelMessage
          ) =
            if (ann.nodeId == FragLNStart.defaultHostedNode.ann.nodeId)
          ...
          override def onMessage(
            ...
            case error: wire.Error
                if nodeId == FragLNStart.defaultHostedNode.ann.nodeId =>
              translateHostedTaggedError(error)
            case upd: ChannelUpdate
                if nodeId == FragLNStart.defaultHostedNode.ann.nodeId && upd.isHosted =>
        ...
        lazy val chainListener = new BlocksListener {
          def onBlocksDownloaded(
            ...
            ConnectionManager
              .connectTo(FragLNStart.defaultHostedNode.ann, notify = true)
        ...
        def detachAll(retryOnRestart: Boolean): Unit = {
          app.prefs.edit
            .putBoolean(AbstractKit.AUTO_HOSTED_CHAN, retryOnRestart)
            .commit
    ...
    if (showTooltip) try {
      ...
      val tip = new SimpleTooltip.Builder(me)
        .anchorView(floatingActionMenu.getMenuIconView)
      tip
        .text("Menu")
    ...
    if (app.isAlive) {
      ...
      PaymentInfoWrap.newRoutesOrGiveUp = rd =>
        if (rd.callsLeft > 0 && ChannelManager.checkIfSendable(rd).isRight) {
          ...
          PaymentInfoWrap fetchAndSend rd.copy(
            callsLeft = rd.callsLeft - 1,
            useCache = false
          )
        } else {
          ...
          val allChanFailsFromPeers = PaymentInfo.errors(rd.pr.paymentHash) forall {
            ...
            case DecryptedFailurePacket(origin, _: TemporaryChannelFailure) \ route =>
              route.size > 1 && origin == rd.nextNodeId(route)
            case DecryptedFailurePacket(origin, PermanentChannelFailure) \ route =>
              route.size > 1 && origin == rd.nextNodeId(route)
            case _ => false
          }

          if (allChanFailsFromPeers)
            UITask(me toast err_ln_peer_can_not_route).run
      ...
      PaymentInfoWrap.failOnUI = rd => {
        ...
        PaymentInfoWrap.updStatus(PaymentInfo.FAILURE, rd.pr.paymentHash)
        if (rd.expensiveScids.nonEmpty)
          UITask(me toast ln_fee_expensive_omitted).run
  ...
  def readNdefMessage(nfcMessage: Message) =
    <(
      ...
      error => app quickToast err_nothing_useful
    )(ok => checkTransData)
  ...
  def checkTransData = app.TransData checkAndMaybeErase {
    ...
    case btcURI: BitcoinURI =>
      val canSendOffChain = Try(btcURI.getAmount)
        .map(coin2MSat)
        .filter(msat => ChannelManager.estimateAIRCanSend >= msat.amount)
        .isSuccess
      if (canSendOffChain && btcURI.getLightningRequest != null)
        <(app.TransData recordValue btcURI.getLightningRequest, onFail)(
          _ => checkTransData
        )
  ...
  def fetch1stLevelUrl(lnUrl: LNUrl) = {
    ...
    <(
      to[LNUrlData](LNUrl guardResponse sslAwareRequest.body).validate(lnUrl),
      onFail
    ) {
      case payReq: PayRequest =>
        FragWallet.worker.lnurlPayOffChainSend(lnUrl.uri.getHost, payReq)
      case withdrawReq: WithdrawRequest =>
        me doReceivePayment Some(withdrawReq, lnUrl)
  ...
  def initIncoming(incoming: IncomingChannelRequest) = {
    val initialListener = new ConnectionListener { self =>
      ...
      override def onOperational(nodeId: PublicKey, isCompatible: Boolean) =
        if (isCompatible) {
          queue
            .map(_ => incoming.requestChannel.body)
            .map(LNUrl.guardResponse)
            .foreach(none, onCallFailed)
        }

      override def onMessage(nodeId: PublicKey, msg: LightningMessage) =
        msg match {
          case open: OpenChannel if !open.channelFlags.isPublic =>
  ...
  def showLoginForm(lnUrl: LNUrl) = lnUrl.k1 foreach { k1 =>
    ...
    def wut(alert: AlertDialog): Unit = {
      val bld = baseTextBuilder(
        getString(ln_url_info_login)
          .format(lnUrl.uri.getHost, linkingPubKey)
          .html
      )
      mkCheckFormNeutral(
        _.dismiss,
        none,
        _ => me share linkingPubKey,
        bld,
        dialog_ok,
        -1,
        dialog_share
    ...
    def doLogin(alert: AlertDialog) = rm(alert) {
      ...
      val secondLevelRequestUri = lnUrl.uri.buildUpon
        .appendQueryParameter("sig", sig.toHex)
        .appendQueryParameter("key", linkingPubKey)
      val sslAwareSecondRequest = get(
        secondLevelRequestUri.build.toString,
        false
      ).connectTimeout(15000).trustAllCerts.trustAllHosts
      queue
        .map(_ => sslAwareSecondRequest.body)
        .map(LNUrl.guardResponse)
        .foreach(_ => onLoginSuccess.run, onFail)
      app quickToast ln_url_resolving
    }
    ...
    def onLoginSuccess = UITask {
      ...
      mkCheckForm(
        ...
        none,
        baseTextBuilder(message),
        dialog_close,
        -1
      )
    }

    val title = updateView2Blue(
      oldView = str2View(new String),
      s"<big>${lnUrl.uri.getHost}</big>"
    )
    mkCheckFormNeutral(
      doLogin,
      none,
      wut,
      baseBuilder(title, null),
      dialog_login,
      dialog_cancel,
      dialog_info
    )
  ...
  def doReceivePayment(extra: Option[RequestAndLNUrl] = None) = {
    ...
    val withRoutes =
      viableChannels.filter(isOperational).flatMap(channelAndHop).toMap
    ...
    val humanShouldSpend =
      s"<strong>${denom parsedWithSign -maxCanReceive}</strong>"
    val reserveUnspentWarning = getString(ln_receive_reserve) format humanShouldSpend

    extra match {
      case Some(wr \ lnUrl) =>
        val title =
          updateView2Blue(str2View(new String), app getString ln_receive_title)
        val finalMaxCanReceiveCapped = MilliSatoshi(
          wr.maxWithdrawable min maxCanReceive.amount
        )

        if (viableChannels.isEmpty)
          showForm(
            negTextBuilder(dialog_ok, getString(ln_receive_howto).html).create
          )
        else if (withRoutes.isEmpty)
          showForm(
            negTextBuilder(dialog_ok, getString(ln_receive_6conf).html).create
          )
        else if (maxCanReceive.amount < 0L)
          showForm(negTextBuilder(dialog_ok, reserveUnspentWarning.html).create)
        else
          FragWallet.worker.receive(
            withRoutes,
            finalMaxCanReceiveCapped,
            wr.minCanReceive,
            title,
            wr.defaultDescription
          ) { rd =>
            queue
              .map(_ => wr.requestWithdraw(lnUrl, rd.pr).body)
              .map(LNUrl.guardResponse)
              .foreach(none, onRequestFailed)
            def onRequestFailed(response: Throwable) =
              wrap(PaymentInfoWrap failOnUI rd)(me onFail response)
          }
      ...
      case None =>
        ...
        val actions = Array(
          getString(ln_receive_option).format(alertLNHint).html,
          getString(btc_receive_option).html
        )
        val lst \ alert =
          makeChoiceList(actions, me getString action_coins_receive)
        ...
        def offChain = rm(alert) {
          if (viableChannels.isEmpty)
            showForm(
              negTextBuilder(dialog_ok, app.getString(ln_receive_howto).html).create
            )
          else
            FragWallet.worker.receive(
              withRoutes,
              maxCanReceive,
              MilliSatoshi(1L),
              app.getString(ln_receive_title).html,
              new String
            ) { rd =>
              app.foregroundServiceIntent
                .putExtra(
                  AwaitService.SHOW_AMOUNT,
                  denom asString rd.pr.amount.get
                )
                .setAction(AwaitService.SHOW_AMOUNT)
              ContextCompat
                .startForegroundService(app, app.foregroundServiceIntent)
              me PRQR rd.pr
            }
        }
  ...
  def goSendPaymentForm(top: View) = {
    ...
    val lst \ alert = makeChoiceList(
      actions.map(getString).map(_.html),
      me getString action_coins_send
    )
    lst setOnItemClickListener onTap {
      case 0 => pastePaymentRequest
      case 1 => depositHivemind
    }

    def pastePaymentRequest = rm(alert) {
      def mayResolve(rawBufferString: String) =
        <(app.TransData recordValue rawBufferString, onFail)(
          _ => checkTransData
        )
      Try(app.getBufferUnsafe) match {
        case Success(rawData) => mayResolve(rawData)
        case _                => app quickToast err_nothing_useful
      }
    }
    ...
    def depositHivemind = rm(alert) {
      ...
      val alert = showForm(
        negTextBuilder(dialog_ok, getString(hivemind_details).html).create
      )
      try Utils clickableTextField alert.findViewById(android.R.id.message)
      catch none
    }
  }
