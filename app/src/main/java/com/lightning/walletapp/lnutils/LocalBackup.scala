...
object LocalBackup {
  ...
  def getBackupFileUnsafe(channelBackupDirName: String, force: Boolean) = {
    val publicDir =
      new File(Environment.getExternalStorageDirectory, BACKUP_DIR)
  ...
  def encryptAndWrite(
    ...
    val utxoMsecOpt = Try {
      if (unspentUtxos.isEmpty) System.currentTimeMillis
      else
        unspentUtxos.map { utxo =>
          utxo.getParentTransaction.getUpdateTime.getTime
        }.min
    }
    val channelsMsecOpt = Try {
      datas.map {
        case cd: ClosingData         => cd.closedAt
        case wd: WaitFundingDoneData => wd.commitments.startedAt
        case _                       => System.currentTimeMillis
      }.min
    }
    val earliest = math.min(
      channelsMsecOpt getOrElse System.currentTimeMillis,
      utxoMsecOpt getOrElse wallet.getEarliestKeyCreationTime * 1000L
    ) / 1000L - 3600 * 24 * 7

    val backup =
      (LocalBackups(Vector.empty, Vector.empty, earliest, v = 1) /: datas) {
        case (LocalBackups(normal, hosted, _, v), hasNorm: HasNormalCommits) =>
          LocalBackups(normal :+ hasNorm, hosted, earliest, v)
        case (
            LocalBackups(normal, hosted, _, v),
            hostedCommits: HostedCommits
            ) =>
          LocalBackups(normal, hosted :+ hostedCommits, earliest, v)
  ...
  def readAndDecrypt(file: File, secret: ByteVector) =
    for {
      encryptedlOCALBackupByteArray <- Try(Files toByteArray file)
      plaintextJson <- AES.decBytes(
        encryptedlOCALBackupByteArray,
        secret.toArray
      )
