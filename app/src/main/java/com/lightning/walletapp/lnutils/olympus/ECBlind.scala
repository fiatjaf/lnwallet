...
case class BlindMemo(
    ...
    clears: List[BigInteger],
    key: String
) {
  def makeBlindTokens = params zip clears map {
    case (param, token) => param.blind(token).toString
  }
  def makeClearSigs(blind: BigInteger*) = params zip blind map {
    case (param, sig) => param unblind sig
  }
...
case class BlindParam(
  ...
  def keyBigInt: BigInteger =
    ECKey.CURVE.getCurve.decodePoint(point).getAffineXCoord.toBigInteger
