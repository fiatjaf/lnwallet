...
class SettingsActivity extends TimerActivity with HumanTimeDisplay { me =>
  lazy val useTrustedNode = findViewById(R.id.useTrustedNode)
    .asInstanceOf[CheckBox]
  lazy val saveLocalBackups = findViewById(R.id.saveLocalBackups)
    .asInstanceOf[CheckBox]
  lazy val fpAuthentication = findViewById(R.id.fpAuthentication)
    .asInstanceOf[CheckBox]
  lazy val constrainLNFees = findViewById(R.id.constrainLNFees)
    .asInstanceOf[CheckBox]

  lazy val useTrustedNodeState = findViewById(R.id.useTrustedNodeState)
    .asInstanceOf[TextView]
  lazy val constrainLNFeesState = findViewById(R.id.constrainLNFeesState)
    .asInstanceOf[TextView]
  lazy val saveLocalBackupsPath = findViewById(R.id.saveLocalBackupsPath)
    .asInstanceOf[TextView]

  lazy val chooseBitcoinUnit = findViewById(R.id.chooseBitcoinUnit)
    .asInstanceOf[Button]
  lazy val recoverFunds = findViewById(R.id.recoverChannelFunds)
    .asInstanceOf[Button]
  lazy val setFiatCurrency = findViewById(R.id.setFiatCurrency)
    .asInstanceOf[Button]
  ...
  override def onRequestPermissionsResult(
    ...
    if (isAllowed)
      ChannelManager.backupSaveWorker.replaceWork("SETTINGS-INIT-SAVE-BACKUP")
    updateBackupView
  }

  def onBackupTap(cb: View) = {
    val isAllowed = LocalBackup.isAllowed(activity = me)
    if (!isAllowed) LocalBackup.askPermission(activity = me)
    else {
      val intent =
        (new Intent).setAction(Settings.ACTION_APPLICATION_DETAILS_SETTINGS)
      val intent1 = intent setData android.net.Uri
        .fromParts("package", getPackageName, null)
      startActivity(intent1)
    }
  }

  def onFpTap(cb: View) = fpAuthentication.isChecked match {
    case true if VERSION.SDK_INT < VERSION_CODES.M =>
      runAnd(fpAuthentication setChecked false)(app quickToast fp_no_support)
    case true if !gf.hasFingerprintHardware =>
      runAnd(fpAuthentication setChecked false)(app quickToast fp_no_support)
    case true if !gf.hasEnrolledFingerprint =>
      runAnd(fpAuthentication setChecked false)(app quickToast fp_add_print)
  ...
  def onTrustedTap(cb: View) = {
    ...
    def addAttempt(alert: AlertDialog) =
      <(process(serverHostPort.getText.toString), onFail)(_ => alert.dismiss)
    alert setOnDismissListener new OnDismissListener {
      def onDismiss(some: DialogInterface) = updateTrustedView
    }
    content
      .findViewById(R.id.serverBackup)
      .asInstanceOf[CheckBox] setVisibility View.GONE
    app.kit.trustedNodeTry.foreach(serverHostPort setText _.toString)
    formatInputHint setText trusted_hint

    def process(rawText: String) =
      if (rawText.nonEmpty) {
        val hostOrIP \ port = rawText.splitAt(rawText lastIndexOf ':')
        val nodeAddress =
          NodeAddress.fromParts(hostOrIP, port.tail.toInt, orElse = Domain)
        app.kit.wallet
          .setDescription(nodeaddress.encode(nodeAddress).require.toHex)
  ...
  def onConstrainLNFeesTap(cb: View) = wrap(updateConstrainLNFeesView) {
    app.prefs.edit
      .putBoolean(AbstractKit.CAP_LN_FEES, constrainLNFees.isChecked)
      .commit
  }
    ...
    if (app.isAlive) {
      ...
      setFiatCurrency setOnClickListener onButtonTap {
        ...
        lst setAdapter new ArrayAdapter(
          ...
          fiatHumanNames.toArray
        )
        showForm(
          alertDialog =
            negBuilder(dialog_ok, me getString sets_set_fiat, form).create
        )
      ...
      chooseBitcoinUnit setOnClickListener onButtonTap {
        ...
        showForm(
          alertDialog =
            negBuilder(dialog_ok, me getString sets_choose_unit, form).create
        )
      ...
      rescanWallet setOnClickListener onButtonTap {
        val bld = baseTextBuilder(me getString sets_rescan_ok)
          .setTitle(me getString sets_rescan)
      ...
      recoverFunds setOnClickListener onButtonTap {
        def recover = app.olympus getBackup cloudId foreach { backups =>
          ...
          for {
            ...
            ref <- AES
              .decBytes(cipherbytes, cloudSecret.toArray) map bin2readable map to[
              RefundingData
            ]
            if !ChannelManager.all
              .flatMap(_.getCommits)
              .map(_.channelId)
              .contains(ref.commitments.channelId)
          } ChannelManager.all +:= ChannelManager
            .createChannel(ChannelManager.operationalListeners, ref)
  ...
  def updateBackupView = {
    ...
    if (canWrite)
      saveLocalBackupsPath.setText(
        LocalBackup.getBackupFileUnsafe(dir, force = true).getPath
      )
  ...
  def updateConstrainLNFeesView = {
    constrainLNFees setChecked app.prefs
      .getBoolean(AbstractKit.CAP_LN_FEES, false)
    val constrainedText = getString(ln_fee_cap_enabled)
      .format("1%", denom parsedWithSign PaymentInfo.onChainThreshold)
    val message =
      if (constrainLNFees.isChecked) constrainedText
      else getString(ln_fee_cap_disabled).format("1%")
