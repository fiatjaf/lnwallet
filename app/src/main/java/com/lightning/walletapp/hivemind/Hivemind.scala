...
object Hivemind {
  ...
  val scriptPubKey = ByteVector.fromValidHex(
  ...
  def madeDepositTx(
      hiveMindHash: ByteVector,
      depositAmount: Satoshi,
      prevTx: Transaction
  ) = {
    val prevDepositIndex =
      new PubKeyScriptIndexFinder(prevTx).findPubKeyScriptIndex(scriptPubKey)
    ...
    rq.tx.addOutput(
      Coin.ZERO,
      ScriptBuilder createOpReturnScript opReturnScript.toArray
    )
    ...
    val currentDepositIndex =
      new PubKeyScriptIndexFinder(rq.tx).findPubKeyScriptIndex(scriptPubKey)
    ...
    input setScriptSig Script.write(
