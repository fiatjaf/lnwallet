...
class WalletApp extends Application { me =>
  ...
  lazy val plur = getString(lang) match {
    ...
    case "chn" | "jpn" =>
      (phraseOptions: Array[String], num: Long) => phraseOptions(1)
    case "rus" | "ukr" =>
  ...
  def isAlive =
    if (null == kit || null == olympus || null == db || null == extendedNodeKey)
      false
    else
      kit.state match {
  ...
  override def onCreate = wrap(super.onCreate) {
    ...
    if (Build.VERSION.SDK_INT > Build.VERSION_CODES.N_MR1) {
      ...
      val srvChan =
        new NotificationChannel(AwaitService.CHANNEL_ID, "NC", importanceLevel)
  ...
  def mkNodeAnnouncement(id: PublicKey, na: NodeAddress, alias: String) =
    NodeAnnouncement(
      signature = sign(Zeroes, randomPrivKey),
  ...
  def emptyRD(pr: PaymentRequest, firstMsat: Long, useCache: Boolean) = {
    ...
    RoutingData(
      pr,
      routes = Vector.empty,
      usedRoute = Vector.empty,
      PacketAndSecrets(emptyOnionPacket, Vector.empty),
      firstMsat = firstMsat,
      lastMsat = 0L,
      lastExpiry = 0L,
      callsLeft = if (useOnChainFeeCap) 8 else 4,
      useCache = useCache,
      airLeft = 0,
      capFeeByOnChain = useOnChainFeeCap,
      expensiveScids = Vector.empty,
      action = None,
      fromHostedOnly = false
    )
  }
  ...
  object TransData {
    ...
    private[this] val lnUrl =
      s"(?im).*?(lnurl)([0-9]{1,}[a-z0-9]+){1}".r.unanchored
    private[this] val lnPayReq =
    ...
    def parse(rawInputTextToParse: String) =
      rawInputTextToParse take 2880 match {
        case bitcoinUriLink if bitcoinUriLink startsWith "bitcoin" =>
          bitcoinUri(bitcoinUriLink)
        case bitcoinUriLink if bitcoinUriLink startsWith "BITCOIN" =>
          bitcoinUri(bitcoinUriLink.toLowerCase)
        case nodeLink(key, host, port) =>
          mkNodeAnnouncement(
            PublicKey(ByteVector fromValidHex key),
  ...
  abstract class WalletKit extends AbstractKit {
    ...
    def conf0Balance =
      wallet getBalance BalanceType.ESTIMATED_SPENDABLE // Returns all utxos
    ...
    def trustedNodeTry =
      Try(
        nodeaddress
          .decode(BitVector fromValidHex wallet.getDescription)
          .require
          .value
      )
    def fundingPubScript(some: HasNormalCommits) =
      singletonList(
        some.commitments.commitInput.txOut.publicKeyScript: org.bitcoinj.script.Script
      )
    def closingPubKeyScripts(cd: ClosingData) =
      cd.commitTxs
        .flatMap(_.txOut)
        .map(_.publicKeyScript: org.bitcoinj.script.Script)
        .asJava
    def useCheckPoints(time: Long) =
      CheckpointManager.checkpoint(
        params,
        getAssets open "checkpoints.txt",
        store,
        time
      )
    ...
    def setupAndStartDownload = {
      ...
      for {
        ...
        if offlineChannel.data.announce.addresses.headOption
          .forall(_.canBeUpdatedIfOffline)
...
object ChannelManager extends Broadcaster {
  ...
  val socketEventsListener = new ConnectionListener {
    ...
    override def onMessage(nodeId: PublicKey, msg: LightningMessage) =
      msg match {
        case channelUpdate: ChannelUpdate =>
          fromNode(nodeId).foreach(_ process channelUpdate)
        case nodeLevelError: Error if nodeLevelError.channelId == Zeroes =>
          fromNode(nodeId).foreach(_ process nodeLevelError)
        case cm: ChannelMessage =>
          fromNode(nodeId)
            .find(_.getCommits.map(_.channelId) contains cm.channelId)
            .foreach(_ process cm)
        case _ =>
      }
    ...
    override def onHostedMessage(
      ...
      fromNode(ann.nodeId)
        .find(_.getCommits.map(_.channelId) contains ann.hostedChanId)
  ...
  val chainEventsListener = new TxTracker with BlocksListener {
    ...
    def onBlocksDownloaded(peer: Peer, b: Block, fb: FilteredBlock, left: Int) =
      onBlock(left)
    def onCoinsReceived(wallet: Wallet, txj: Transaction, a: Coin, b: Coin) =
      onChainTx(txj)
    def onCoinsSent(wallet: Wallet, txj: Transaction, a: Coin, b: Coin) =
      onChainTx(txj)
  ...
  def blockDaysLeft: Int =
    currentBlocksLeft.map(_ / blocksPerDay).getOrElse(Int.MaxValue)
  def currentHeight: Int =
    app.kit.wallet.getLastBlockSeenHeight + currentBlocksLeft.getOrElse(0)
  ...
  override def onProcessSuccess = {
    case (_: NormalChannel, close: ClosingData, _: Command) =>
      ...
      val tier12Publishable =
        for (state <- close.tier12States if state.isPublishable) yield state.txn
      val toSend = close.mutualClose ++ close.localCommit.map(_.commitTx) ++ tier12Publishable
      for (tx <- toSend)
        try app.kit blockSend tx
        catch none

    case (chan: NormalChannel, wait: WaitFundingDoneData, CMDChainTipKnown)
        if wait.our.isEmpty =>
    ...
    case (chan: NormalChannel, ref: RefundingData, CMDChainTipKnown)
        if ref.remoteLatestPoint.isDefined =>
  ...
  override def onBecome = {
    ...
    case (_: NormalChannel, wait: WaitFundingDoneData, _, _) =>
      app.kit blockSend wait.fundingTx
    case (chan: NormalChannel, _: NormalData, SLEEPING, OPEN) =>
  ...
  def delayedPublishes = {
    val statuses = all.map(_.data).collect {
      case cd: ClosingData => cd.bestClosing.getState
    }
    // Select all ShowDelayed which can't be published yet because cltv/csv delays are not cleared on them
    statuses.flatten.collect {
      case sd: ShowDelayed if !sd.isPublishable && sd.delay > Long.MinValue =>
        sd
    }
  }
  ...
  def airCanSendInto(targetChan: Channel) =
    for {
      canSend <- all
        .filter(isOperational)
        .diff(targetChan :: Nil)
        .map(_.estCanSendMsat)
  ...
  def estimateAIRCanSend = {
    ...
    val airCanSend = mostFundedChanOpt.map(
      chan => chan.estCanSendMsat + airCanSendInto(chan).sum
    ) getOrElse 0L
    val usefulCaps = all
      .filter(isOperational)
      .map(
        chan =>
          chan.estCanSendMsat.zeroIfNegative + chan.estCanReceiveMsat.zeroIfNegative
      )
  ...
  def hasNormalChanWith(nodeId: PublicKey) =
    fromNode(nodeId).exists(
      chan => isOpeningOrOperational(chan) && !chan.isHosted
    )
  ...
  def initConnect =
    for (chan <- all if chan.state != CLOSING)
      ConnectionManager.connectTo(chan.data.announce, notify = false)
  def fromNode(nodeId: PublicKey) =
    for (chan <- all if chan.data.announce.nodeId == nodeId) yield chan
  def activeInFlightHashes =
    all.filter(isOperational).flatMap(_.inFlightHtlcs).map(_.add.paymentHash)
  def mostFundedChanOpt =
    all.filter(isOperational).sortBy(_.estCanSendMsat).lastOption
  def attachListener(lst: ChannelListener) =
    for (chan <- all) chan.listeners += lst
  def detachListener(lst: ChannelListener) =
    for (chan <- all) chan.listeners -= lst

  def check2ndLevelSpent(chan: NormalChannel, cd: ClosingData) =
    retry(app.olympus getChildTxs cd.commitTxs.map(_.txid), pickInc, 7 to 8)
      .foreach(
        txs => {
  ...
  def check1stLevelSpent(chan: NormalChannel, some: HasNormalCommits) =
    retry(
      app.olympus getChildTxs Seq(some.commitments.commitInput.outPoint.txid),
      pickInc,
      7 to 8
    ).foreach(
      txs => {
  ...
  val backupSaveWorker = new ThrottledWork[String, Any] {
    ...
    def process(cmd: String, execResult: Any) =
      if (LocalBackup.isExternalStorageWritable) try {
        val backupFile = LocalBackup.getBackupFileUnsafe(
          LocalBackup getBackupDirectory LNParams.chainHash,
          force = true
        )
        LocalBackup.encryptAndWrite(
          backupFile,
          for (chan <- all) yield chan.data,
          app.kit.wallet,
          LNParams.cloudSecret
        )
      } catch none
  }

  def createHostedChannel(
      initListeners: Set[ChannelListener],
      bootstrap: ChannelData
  ) = new HostedChannel {
  ...
  def createChannel(
    ...
    def REV(cs: NormalCommits, rev: RevokeAndAck) =
      for {
        ...
        revocationInfo = Helpers.Closing
          .makeRevocationInfo(cs, tx, rev.perCommitmentSecret, watchtowerFee)
        serialized = LightningMessageCodecs.serialize(
          revocationInfoCodec encode revocationInfo
        )
      } db.change(
        RevokedInfoTable.newSql,
        tx.txid,
        cs.channelId,
        myBalance,
        serialized
    ...
    def GETREV(cs: NormalCommits, tx: fr.acinq.bitcoin.Transaction) = {
      ...
      val rc =
        RichCursor(databaseCursor).headTry(_ string RevokedInfoTable.info)
      ...
      for {
        ...
        riWithCurrentFeeRate = ri.copy(
          feeRate = ri.feeRate max broadcaster.perKwThreeSat
        )
        ri1 = Helpers.Closing.reMakeRevocationInfo(
          riWithCurrentFeeRate,
          cs,
          tx,
          perCommitmentSecret
        )
    ...
    def CLOSEANDWATCH(cd: ClosingData) = {
      cd.tier12States.collect { case dps: DelayedPublishStatus => dps.txn.bin }.toVector match {
        case txs if txs.isEmpty =>
          Tools log "Closing contains no second tier txs, nothing to schedule on Olympus"
        case txs =>
          app.olympus tellClouds TxUploadAct(
            txvec.encode(txs).require.toByteVector,
            Nil,
            "txs/schedule"
          )
      }
    ...
    def ASKREFUNDPEER(some: HasNormalCommits, point: Point) = {
      val msg =
        ByteVector.fromValidHex("please publish your local commitment".s2hex)
  ...
  def checkIfSendable(rd: RoutingData) = {
    ...
    val canNotBeSent = paymentInfoOpt.exists(
      info => info.status == SUCCESS || info.status == FROZEN
    )
    if (canNotBeSent) Left(err_ln_fulfilled, NOT_SENDABLE)
    else
      mostFundedChanOpt.map(_.estCanSendMsat) match {
        ...
        case Some(max)
            if max < rd.firstMsat && rd.airLeft > 1 && estimateAIRCanSend >= rd.firstMsat =>
          Left(dialog_sum_big, SENDABLE_AIR)
        case Some(max) if max < rd.firstMsat =>
          Left(dialog_sum_big, NOT_SENDABLE)
  ...
  def fetchRoutes(rd: RoutingData) = {
    val from: PublicKeyVec = rd.fromHostedOnly match {
      case false =>
        all
          .filter(
            chan => isOperational(chan) && chan.estCanSendMsat >= rd.firstMsat
          )
          .map(_.data.announce.nodeId)
          .distinct
      case true =>
        all
          .filter(
            chan =>
              isOperational(chan) && chan.estCanSendMsat >= rd.firstMsat && chan.isHosted
          )
          .map(_.data.announce.nodeId)
          .distinct
    }
    ...
    def getRoutes(target: PublicKey) =
      if (rd.isReflexive) from diff Vector(target) match {
        ...
        case restFrom if rd.useCache =>
          RouteWrap.findRoutes(restFrom, target, rd)
        case restFrom => BadEntityWrap.findRoutes(restFrom, target, rd)
      } else
        from contains target match {
    ...
    for {
      ...
      openMap = Tools.toDefMap[Channel, PublicKey, Int](
        all.filter(isOperational),
        _.data.announce.nodeId,
        chan => if (chan.state == OPEN) 0 else 1,
        default = 1
      )
      busyMap = Tools.toDefMap[Channel, PublicKey, Int](
        all.filter(isOperational),
        _.data.announce.nodeId,
        chan => chan.inFlightHtlcs.size,
        default = 1
      )
      foundRoutes = rs
        .sortBy(totalRouteFee(_, 10000000L))
        .sortBy(openMap compose rd.nextNodeId)
        .sortBy(busyMap compose rd.nextNodeId)
      // We may have out of band pre-filled routes in RD so append them to found ones
    } yield useFirstRoute(foundRoutes ++ rd.routes, rd)
  }

  def sendEither(foeRD: FullOrEmptyRD, noRoutes: RoutingData => Unit): Unit =
    foeRD match {
      ...
      case Right(rd) =>
        all.filter(isOperational) find { chan =>
          val matchesHostedOnlyPolicy = if (rd.fromHostedOnly) chan.isHosted
          else true // User may desire to only spend from hosted channels
          val correctTargetPeerNodeId = chan.data.announce.nodeId == rd
            .nextNodeId(rd.usedRoute) // Onion does not care about specific chan but peer nodeId must match
          val notLoop = chan.getCommits
            .flatMap(_.updateOpt)
            .map(_.shortChannelId) != rd.usedRoute.lastOption
            .map(_.shortChannelId) // Not chans like A -> B -> A
...
object Vibrator {
  ...
  def canVibrate =
    null != vib && vib.hasVibrator && lastVibrated < System.currentTimeMillis - 3000L
