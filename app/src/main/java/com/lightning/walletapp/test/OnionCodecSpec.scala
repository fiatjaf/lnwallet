...
class OnionCodecSpec {
  ...
  def allTests = {
...

    {
      ...
      val testCases = Map(
        RelayLegacyPayload(0, 0, 0) -> ByteVector.fromValidHex(
          "00 0000000000000000 0000000000000000 00000000 000000000000000000000000"
        ),
        RelayLegacyPayload(42, 142000, 500000) -> ByteVector.fromValidHex(
          "00 000000000000002a 0000000000022ab0 0007a120 000000000000000000000000"
        ),
        RelayLegacyPayload(561, 1105, 1729) -> ByteVector.fromValidHex(
          "00 0000000000000231 0000000000000451 000006c1 000000000000000000000000"
        )
      )
...

    {
      ...
      val testCases = Map(
        FinalLegacyPayload(0, 0) -> ByteVector.fromValidHex(
          "00 0000000000000000 0000000000000000 00000000 000000000000000000000000"
        ),
        FinalLegacyPayload(142000, 500000) -> ByteVector.fromValidHex(
          "00 0000000000000000 0000000000022ab0 0007a120 000000000000000000000000"
        ),
        FinalLegacyPayload(1105, 1729) -> ByteVector.fromValidHex(
          "00 0000000000000000 0000000000000451 000006c1 000000000000000000000000"
        )
      )
...

    {
      ...
      for ((payloadLength, bin) <- testCases) {
        assert(
          payloadLengthDecoder.decode(bin.bits).require.value == payloadLength
        )
      }
    }

    {
      ...
      val testCases = Map(
        TlvStream[OnionTlv](
          AmountToForward(561),
          OutgoingCltv(42),
          OutgoingChannelId(1105)
        ) -> ByteVector.fromValidHex("11 02020231 04012a 06080000000000000451"),
        TlvStream[OnionTlv](
          Seq(AmountToForward(561), OutgoingCltv(42), OutgoingChannelId(1105)),
          Seq(GenericTlv(UInt64(65535), ByteVector.fromValidHex("06c1")))
        ) -> ByteVector.fromValidHex(
          "17 02020231 04012a 06080000000000000451 fdffff0206c1"
        )
      )
      ...
      for ((expected, bin) <- testCases) {
        ...
        val encoded = relayPerHopPayloadCodec
          .encode(RelayTlvPayload(expected))
          .require
          .bytes
        assert(encoded == bin)
      }
...

    {
      ...
      val testCases = Map(
        TlvStream[OnionTlv](AmountToForward(561), OutgoingCltv(42)) -> ByteVector
          .fromValidHex("07 02020231 04012a"),
        TlvStream[OnionTlv](
          AmountToForward(561),
          OutgoingCltv(42),
          OutgoingChannelId(1105)
        ...
        TlvStream[OnionTlv](
          Seq(AmountToForward(561), OutgoingCltv(42)),
          Seq(GenericTlv(UInt64(65535), ByteVector.fromValidHex("06c1")))
        ) -> ByteVector.fromValidHex("0d 02020231 04012a fdffff0206c1")
      )
      ...
      for ((expected, bin) <- testCases) {
        ...
        val encoded = finalPerHopPayloadCodec
          .encode(FinalTlvPayload(expected))
          .require
          .bytes
        assert(encoded === bin)
      }
    ...
    {
      println(
        "decode variable-length (tlv) relay per-hop payload missing information"
      )
      val testCases = Seq(
        (
          InvalidOnionPayload(UInt64(2), 0),
          ByteVector.fromValidHex("0d 04012a 06080000000000000451")
        ), // missing amount
        (
          InvalidOnionPayload(UInt64(4), 0),
          ByteVector.fromValidHex("0e 02020231 06080000000000000451")
        ), // missing cltv
        (
          InvalidOnionPayload(UInt64(6), 0),
          ByteVector.fromValidHex("07 02020231 04012a")
        ) // missing channel id
      )
    ...
    {
      println(
        "decode variable-length (tlv) final per-hop payload missing information"
      )
      val testCases = Seq(
        (
          InvalidOnionPayload(UInt64(2), 0),
          ByteVector.fromValidHex("03 04012a")
        ), // missing amount
        (
          InvalidOnionPayload(UInt64(4), 0),
          ByteVector.fromValidHex("04 02020231")
        ) // missing cltv
      )
...

    {
      ...
      val testCases = Seq(
        ...
        ByteVector
          .fromValidHex("00 000000000000002a 000000000000002a"), // invalid length
        // Invalid variable-length (tlv) payload.
        ByteVector
          .fromValidHex("00"), // empty payload is missing required information
