...
class FailureMessageLightningMessageCodecsSpec {
  ...
  val channelUpdate = ChannelUpdate(
    signature = ByteVector.fromValidHex(
      "3045022100c451cd65c88f55b1767941a247e849e12f5f4d4a93a07316659e22f5267d2088022009042a595c6bc8942cd9d729317b82b306edc259fb6b3a3cecb3dd1bd446e90601"
    ),
  ...
  def allTests = {
    ...
    for ((node, perm, msg) <- msgs) {
      ...
      val decoded =
        FailureMessageCodecs.failureMessageCodec.decode(encoded).require.value
      assert(msg == decoded)
    ...
    println(
      "support encoding of channel_update with/without type in failure messages"
    )
    ...
    val ref = TemporaryChannelFailure(
      ChannelUpdate(
        ByteVector.fromValidHex(
          "3045022100cc3e80149073ed487c76e48e9622bf980f78267b8a34a3f61921f2d8fce6063b022008e74f34a073a13f2097337e4915bb4c001f3b5c4d81e9524ed575e1f457821901"
        ),
    ...
    val u = FailureMessageCodecs.failureMessageCodec
      .decode(tmp_channel_failure_notype.toBitVector)
      .require
      .value
    assert(u == ref)
    val bin = ByteVector.view(
      FailureMessageCodecs.failureMessageCodec.encode(u).require.toByteArray
    )
    assert(bin == tmp_channel_failure_withtype)
    val u2 = FailureMessageCodecs.failureMessageCodec
      .decode(BitVector(bin))
      .require
      .value
    assert(u2 == ref)
    ...
    for ((node, perm, bin) <- testCases) {
      val decoded =
        FailureMessageCodecs.failureMessageCodec.decode(bin.bits).require.value
...

    {
      ...
      val msgs = Map(
        (FailureMessageCodecs.BADONION | FailureMessageCodecs.PERM | 4) -> InvalidOnionVersion(
          randomBytes(32)
        ),
        (FailureMessageCodecs.BADONION | FailureMessageCodecs.PERM | 5) -> InvalidOnionHmac(
          randomBytes(32)
        ),
        (FailureMessageCodecs.BADONION | FailureMessageCodecs.PERM | 6) -> InvalidOnionKey(
          randomBytes(32)
        ),
...

    {
      ...
      val codec =
        FailureMessageCodecs.failureOnionCodec(Hmac256(Protocol.Zeroes))
      val testCases = Map(
        InvalidOnionKey(
          ByteVector.fromValidHex(
...

    {
      ...
      val testCases = Seq(
        ...
        ByteVector.fromValidHex(
          "8c92256e45bbe765130d952e6c043cf594ab25224701f5477fce0e50ee88fa21 0002 4001 0002 "
        ),
        // Padding too big.
        ByteVector.fromValidHex(
        ...
        // Padding length doesn't match actual padding.
        ByteVector.fromValidHex(
          "3898307b7c01781628ff6f854a4a78524541e4afde9b44046bdb84093f082d9d 0002 4001 00ff "
        )
      )
