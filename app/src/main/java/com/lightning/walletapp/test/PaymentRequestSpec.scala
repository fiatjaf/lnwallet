...
class PaymentRequestSpec {
  ...
  def allTests = {
...

    {
      ...
      assert(
        pr.paymentHash == ByteVector.fromValidHex(
...

    {
      ...
      assert(
        pr.description == "3925b6f67e2c340036ed12093dd44e0368df1b6ea26c53dbe4811f58fd5db8c1"
      )

      assert(
        pr.routingInfo == Vector(
          RoutingInfoTag(
            Vector(
              Hop(
                PublicKey(
      ...
      assert(
        Protocol
          .writeUInt64(0x0102030405060708L, ByteOrder.BIG_ENDIAN) == ByteVector
          .fromValidHex("0102030405060708")
      )
      assert(
        Protocol
          .writeUInt64(0x030405060708090AL, ByteOrder.BIG_ENDIAN) == ByteVector
          .fromValidHex("030405060708090a")
      )
    ...
    {
      println(
        "On mainnet, with fallback (p2wsh) address bc1qrp33g0q5c5txsp9arysrx4k6zdkfs4nce4xj0gdcccefvpysxf3qccfmv3 and a minimum htlc cltv expiry of 12"
      )
      val ref =
    ...
    {
      println(
        "On mainnet, please send $30 for coffee beans to the same peer, which supports features 1 and 9"
      )
      val ref =
      ...
      assert(
        pr.paymentHash.toHex == "0001020304050607080900010203040506070809000102030405060708090102"
      )
      assert(pr.timestamp == 1496314658L)
      assert(
        pr.nodeId.toString == "03e7156ae33b0a208d0744199163177e909e80176e55d97a2f221ede0f934dd9ad"
      )
