...
class TransactionsSpec {
  def allTests = {
    ...
    {
      val htlcTx = Transaction.read(
        "02000000014ae539d5b7967b50fc3071f5460927275d195e46868dc81a2cf400256572f3ca000000000000000000012a6b010000000000220020633a5f65a0247fa919f14ceab9ff5afd00cf9de5a36b09530d53facb9a43ea7300000000"
      )
      val dustLimit = Satoshi(1100L)
      val remoteRevocationPubkey = PublicKey(
        ByteVector.fromValidHex(
          "02fead8896083932987c1293a34826a5293abab9ead093280cb78283434a2767da"
        )
      )
      val toSelfDelay = 144
      val remoteDelayedPaymentPubkey = PublicKey(
        ByteVector.fromValidHex(
          "0221c61fca26c469d6e64ae22b10173767f4ac26a0eac3187e601ea9639c1ab960"
        )
      )
      val defaultFinalScriptPubKey =
        ByteVector.fromValidHex("0014888dbbd7998f9f80d4f82b4f9ce6d5882f310aec")
      val feeratePerKwPenalty = 10000L
      val resultPenaltyTx = Transaction.read(
        "0200000001fdefba99dc8708aff81e1479c454dac7d6e1eb689b44dee43915f4a4882e947c0000000000ffffffff013858010000000000160014888dbbd7998f9f80d4f82b4f9ce6d5882f310aec00000000"
      )
      val singnedPenaltyTx = Transaction.read(
        ""
      )
      val remotePerCommitmentSecret = Scalar(
        ByteVector.fromValidHex(
          "4fedc052c833811ff298ebad5107911a394d0f87d0113554377f822b9f8b9417"
        )
      )
      val remoteRevocationPrivkey = revocationPrivKey(
        PrivateKey(
          ByteVector.fromValidHex(
            "e1d4a574e8757e0ce365410ff57f1cf82d37a63d3c87faad02bd85599bc935c101"
          )
        ),
        remotePerCommitmentSecret
      )

      val notSigned = Scripts
...

    {
      ...
      val localRevocationPriv =
        PrivateKey(ByteVector.fromValidHex("cc" * 32), compressed = true)
      val localPaymentPriv =
        PrivateKey(ByteVector.fromValidHex("dd" * 32), compressed = true)
      val localHtlcPriv =
        PrivateKey(ByteVector.fromValidHex("ea" * 32), compressed = true)
      val remoteHtlcPriv =
        PrivateKey(ByteVector.fromValidHex("eb" * 32), compressed = true)
      val finalPubKeyScript = Script.write(
        Script.pay2wpkh(
          PrivateKey(ByteVector.fromValidHex("fe" * 32), compressed = true).publicKey
        )
      )
...

      {
        ...
        val commitTx = Transaction(
          version = 0,
          txIn = Nil,
          txOut = TxOut(Satoshi(20000), pubKeyScript) :: Nil,
          lockTime = 0
        )
        val claimP2WPKHOutputTx = makeClaimP2WPKHOutputTx(
          commitTx,
...

      {
        ...
        val claimHtlcDelayedTx = makeClaimDelayedOutputTx(
        ...
        // we use dummy signatures to compute the weight
        val weight = Transaction.weight(
          addSigs(claimHtlcDelayedTx.get, ByteVector.fromValidHex("bb" * 71)).tx
        )
...

      {
        ...
        val mainPenaltyTx = makeMainPenaltyTx(
          commitTx,
          localRevocationPriv.publicKey,
          finalPubKeyScript,
          toLocalDelay,
        ...
        // we use dummy signatures to compute the weight
        val weight = Transaction.weight(
          addSigs(mainPenaltyTx.get, ByteVector.fromValidHex("bb" * 71)).tx
        )
...

      {
        ...
        val commitTx = Transaction(
          version = 0,
          txIn = Nil,
          txOut = TxOut(Satoshi(htlc.amountMsat / 1000), pubKeyScript) :: Nil,
          lockTime = 0
        )
        val htlcPenaltyTx = makeHtlcPenaltyTx(
          new PubKeyScriptIndexFinder(commitTx),
...

      {
        ...
        val claimHtlcSuccessTx = makeClaimHtlcSuccessTx(
          new PubKeyScriptIndexFinder(commitTx),
          remoteHtlcPriv.publicKey,
          localHtlcPriv.publicKey,
          localRevocationPriv.publicKey,
...

      {
        ...
        val claimClaimHtlcTimeoutTx = makeClaimHtlcTimeoutTx(
          new PubKeyScriptIndexFinder(commitTx),
          remoteHtlcPriv.publicKey,
          localHtlcPriv.publicKey,
          localRevocationPriv.publicKey,
...

    {
      ...
      val localFundingPriv = PrivateKey(
        ByteVector.fromValidHex("a1" * 32) :+ 1.toByte
      )
      val remoteFundingPriv = PrivateKey(
        ByteVector.fromValidHex("a2" * 32) :+ 1.toByte
      )
      val localRevocationPriv = PrivateKey(
        ByteVector.fromValidHex("a3" * 32) :+ 1.toByte
      )
      val localPaymentPriv = PrivateKey(
        ByteVector.fromValidHex("a4" * 32) :+ 1.toByte
      )
      val localDelayedPaymentPriv = PrivateKey(
        ByteVector.fromValidHex("a5" * 32) :+ 1.toByte
      )
      val remotePaymentPriv = PrivateKey(
        ByteVector.fromValidHex("a6" * 32) :+ 1.toByte
      )
      val localHtlcPriv = PrivateKey(
        ByteVector.fromValidHex("a7" * 32) :+ 1.toByte
      )
      val remoteHtlcPriv = PrivateKey(
        ByteVector.fromValidHex("a8" * 32) :+ 1.toByte
      )
      val finalPubKeyScript = Script.write(
        Script.pay2wpkh(
          PrivateKey(ByteVector.fromValidHex("a9" * 32), compressed = true).publicKey
        )
      )
      val commitInput = Funding.makeFundingInputInfo(
        ByteVector.fromValidHex("a0" * 32),
        0,
        Btc(1),
      ...
      val htlc1 = UpdateAddHtlc(
        ByteVector.fromValidHex("00" * 32),
        0,
        millibtc2satoshi(MilliBtc(100)).amount * 1000,
      ...
      val htlc2 = UpdateAddHtlc(
        ByteVector.fromValidHex("00" * 32),
        1,
        millibtc2satoshi(MilliBtc(200)).amount * 1000,
      ...
      val htlc3 = UpdateAddHtlc(
        ByteVector.fromValidHex("00" * 32),
        2,
        (localDustLimit + weight2fee(feeratePerKw, htlcTimeoutWeight)).amount * 1000,
      ...
      val htlc4 = UpdateAddHtlc(
        ByteVector.fromValidHex("00" * 32),
        3,
        (localDustLimit + weight2fee(feeratePerKw, htlcSuccessWeight)).amount * 1000,
      ...
      val commitTx = {
        val txinfo = makeCommitTx(
          commitInput,
          commitTxNumber,
          localPaymentPriv.toPoint,
          remotePaymentPriv.toPoint,
          true,
          localDustLimit,
          localRevocationPriv.publicKey,
          toLocalDelay,
...

      {
        ...
        val hash = Crypto.sha256(
          localPaymentPriv.publicKey.toBin ++ remotePaymentPriv.publicKey.toBin
        )
        val num = Protocol.uint64(
          hash.takeRight(8).toArray,
          ByteOrder.BIG_ENDIAN
        ) & 0xFFFFFFFFFFFFL
        val check = ((commitTx.tx
          .txIn(0)
          .sequence & 0xffffff) << 24) | commitTx.tx.lockTime
...

      {
        ...
        for ((htlc, paymentPreimage) <- (htlc1, paymentPreimage1) :: (
               htlc3,
               paymentPreimage3
             ) :: Nil) {
          val claimHtlcSuccessTx = makeClaimHtlcSuccessTx(
            new PubKeyScriptIndexFinder(commitTx.tx),
            remoteHtlcPriv.publicKey,
            localHtlcPriv.publicKey,
            localRevocationPriv.publicKey,
            finalPubKeyScript,
...

      {
        ...
        for ((htlcSuccessTx, paymentPreimage) <- (
               htlcSuccessTxs(0),
               paymentPreimage2
             ) :: (htlcSuccessTxs(1), paymentPreimage4) :: Nil) {
          val localSig = Scripts.sign(localHtlcPriv)(htlcSuccessTx)
          val remoteSig = Scripts.sign(remoteHtlcPriv)(htlcSuccessTx)
          val signedTx =
            addSigs(htlcSuccessTx, localSig, remoteSig, paymentPreimage)
...

      {
        ...
        val signedTx =
          addSigs(claimP2WPKHOutputTx, localSig, remotePaymentPriv.publicKey)
        assert(checkValid(signedTx).isSuccess)
      }

      {
        // remote spends remote->local htlc output directly in case of timeout
        val claimHtlcTimeoutTx = makeClaimHtlcTimeoutTx(
          new PubKeyScriptIndexFinder(commitTx.tx),
          remoteHtlcPriv.publicKey,
          localHtlcPriv.publicKey,
          localRevocationPriv.publicKey,
    ...
    Future {
      ...
      val bolt3 = Source
        .fromURL(
          "https://raw.githubusercontent.com/lightningnetwork/lightning-rfc/master/03-transactions.md"
        )
        .mkString
        .replace("    name:", "$   name:")
      ...
      val tests = testRegex
        ...
        .map(s => {
          ...
          TestSetup(
            ...
            dustLimit,
            CommitmentSpec(
              feerate_per_kw.toLong,
              to_local_msat.toLong,
              to_remote_msat.toLong,
              htlcs
            ),
            Satoshi(fee.toLong)
          )
      ...
      println(
        tests.size == 15,
        "there were 15 tests at ec99f893f320e8c88f564c1c8566f3454f0f1f5f"
      )
