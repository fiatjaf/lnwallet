...
object Tools {
  ...
  val nextDummyHtlc = UpdateAddHtlc(
    Zeroes,
    id = -1,
    LNParams.minCapacityMsat,
    One,
    expiry = 144 * 3
  )
  def randomPrivKey =
    PrivateKey(ByteVector.view(random getBytes 32), compressed = true)
  def log(consoleMessage: String): Unit =
    android.util.Log.d("LN", consoleMessage)
  def wrap(run: => Unit)(go: => Unit) =
    try go
    catch none
    finally run
  ...
  def toDefMap[T, K, V](
    ...
    val sequenceOfTuples =
      for (mapElement <- source) yield keyFun(mapElement) -> valFun(mapElement)
  ...
  def sign(data: ByteVector, pk: PrivateKey) =
    Try {
  ...
  def toShortIdOpt(
    ...
    val result = blockHeight.&(0xFFFFFFL).<<(40) | txIndex
      .&(0xFFFFFFL)
      .<<(16) | outputIndex.&(0xFFFFL)
  ...
  def hostedChanId(pubkey1: ByteVector, pubkey2: ByteVector) = {
    ...
    if (pubkey1First) Crypto.sha256(pubkey1 ++ pubkey2)
    else Crypto.sha256(pubkey2 ++ pubkey1)
  }
}
...
object Features {
  ...
  implicit def binData2BitSet(featuresBinaryData: ByteVector): util.BitSet =
    util.BitSet.valueOf(featuresBinaryData.reverse.toArray)
  def dataLossProtect(bitset: util.BitSet) =
    bitset.get(OPTION_DATA_LOSS_PROTECT_OPTIONAL) || bitset.get(
      OPTION_DATA_LOSS_PROTECT_MANDATORY
    )
  def variableLengthOnion(bitset: util.BitSet) =
    bitset.get(VARIABLE_LENGTH_ONION_OPTIONAL) || bitset.get(
      VARIABLE_LENGTH_ONION_MANDATORY
    )
  def isBitSet(position: Int, bitField: Byte): Boolean =
    bitField.&(1 << position) == (1 << position)

  def areSupported(bitset: util.BitSet): Boolean = {
    val mandatoryFeatures: Set[Int] = Set(OPTION_DATA_LOSS_PROTECT_MANDATORY)
    def mandatoryUnsupported(n: Int) =
      bitset.get(n) && !mandatoryFeatures.contains(n)
    !(0 until bitset.length by 2 exists mandatoryUnsupported)
  }
}

class LightningException(reason: String = "Lightning related failure")
    extends RuntimeException(reason)
case class CMDAddImpossible(rd: RoutingData, code: Int, hint: Long = 0L)
