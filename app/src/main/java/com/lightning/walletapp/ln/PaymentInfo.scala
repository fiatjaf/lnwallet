...
object PaymentInfo {
  ...
  def buildOnion(
      keys: PublicKeyVec,
      payloads: Vector[PerHopPayload],
      assoc: ByteVector
  ): PacketAndSecrets = {
    require(
      keys.size == payloads.size,
      "Count mismatch: there should be exactly as much payloads as node pubkeys"
    )
    PaymentPacket.create(
      Tools.randomPrivKey,
      keys,
      for (payload <- payloads) yield payload.encode,
      assoc
    )
  }
  ...
  def onChainThreshold =
    Scripts.weight2fee(LNParams.broadcaster.perKwSixSat, 750)
  def useRoute(
      route: PaymentRoute,
      rest: PaymentRouteVec,
      rd: RoutingData
  ): FullOrEmptyRD = {
    ...
    val (allPayloads, nodeIds, lastMsat, lastExpiry) =
      route.reverse.foldLeft(start) {
        case (payloads, nodes, msat, expiry) \ hop =>
          (
            RelayLegacyPayload(hop.shortChannelId, msat, expiry) +: payloads,
            hop.nodeId +: nodes,
            hop.fee(msat) + msat,
            hop.cltvExpiryDelta + expiry
          )
      }

    val isCltvBreach = lastExpiry - LNParams.broadcaster.currentHeight > LNParams.maxCltvDelta
    val isOnChainCapBreach = rd.capFeeByOnChain && MilliSatoshi(
      lastMsat - rd.firstMsat
    ) > onChainThreshold
    val rd1 =
      if (isOnChainCapBreach)
        rd.modify(_.expensiveScids)
          .using(_ :+ route.maxBy(_ fee 10000000L).shortChannelId)
      else rd
    val isUnacceptableRoute = LNParams.isFeeBreach(
      route,
      rd.firstMsat,
      percent = 100L
    ) || isOnChainCapBreach || isCltvBreach

    if (isUnacceptableRoute) useFirstRoute(rest, rd1)
    else {
      val onion = buildOnion(
        keys = nodeIds :+ rd1.pr.nodeId,
        payloads = allPayloads,
        assoc = rd1.pr.paymentHash
      )
      val rd2 = rd1.copy(
        routes = rest,
        usedRoute = route,
  ...
  def failIncorrectDetails(
    ...
    failHtlc(
      ...
      IncorrectOrUnknownPaymentDetails(
        msat.toLong,
        LNParams.broadcaster.currentHeight
      ),
      add
    )
  ...
  def failHtlc(
    ...
    CMDFailHtlc(
      reason = FailurePacket.create(packet.sharedSecret, msg),
      id = add.id
    )

  def withoutChan(shortId: Long, rd: RoutingData, span: Long, msat: Long) = {
    val routesWithoutBadChannels =
      without(rd.routes, _.shortChannelId == shortId)
  ...
  def withoutNodes(badNodes: PublicKeyVec, rd: RoutingData, span: Long) = {
    ...
    val blackListedNodes =
      for (node <- badNodes) yield (node.toString, span, 0L)
  ...
  def parseFailureCutRoutes(fail: UpdateFailHtlc)(rd: RoutingData) = {
    ...
    parsed.map {
      case DecryptedFailurePacket(nodeKey, _: Perm)
          if nodeKey == rd.pr.nodeId =>
        None -> Vector.empty
      case DecryptedFailurePacket(nodeKey, ExpiryTooFar)
          if nodeKey == rd.pr.nodeId =>
        None -> Vector.empty
      case DecryptedFailurePacket(nodeKey, u: ExpiryTooSoon)
          if !replacedChans.contains(u.update.shortChannelId) =>
        replaceChan(nodeKey, rd, u.update)
      case DecryptedFailurePacket(nodeKey, u: FeeInsufficient)
          if !replacedChans.contains(u.update.shortChannelId) =>
        replaceChan(nodeKey, rd, u.update)
      case DecryptedFailurePacket(nodeKey, u: IncorrectCltvExpiry)
          if !replacedChans.contains(u.update.shortChannelId) =>
        replaceChan(nodeKey, rd, u.update)
      ...
      case DecryptedFailurePacket(nodeKey, u: Update) =>
        ...
        if (!isHonest) withoutNodes(Vector(nodeKey), rd, 86400 * 7 * 1000)
          ...
          rd.usedRoute.collectFirst {
            case payHop if payHop.nodeId == nodeKey =>
              withoutChan(u.update.shortChannelId, rd, 180 * 1000, rd.firstMsat)
          } getOrElse withoutNodes(Vector(nodeKey), rd, 180 * 1000)

      case DecryptedFailurePacket(nodeKey, PermanentNodeFailure) =>
        withoutNodes(Vector(nodeKey), rd, 86400 * 7 * 1000)
      case DecryptedFailurePacket(nodeKey, RequiredNodeFeatureMissing) =>
        withoutNodes(Vector(nodeKey), rd, 86400 * 1000)
      case DecryptedFailurePacket(nodeKey, _: BadOnion) =>
        withoutNodes(Vector(nodeKey), rd, 180 * 1000)

      case DecryptedFailurePacket(
          nodeKey,
          UnknownNextPeer | PermanentChannelFailure
          ) =>
        rd.usedRoute.collectFirst {
          case payHop if payHop.nodeId == nodeKey =>
  ...
  def doResolve(
    ...
    (
      LightningMessageCodecs.finalPerHopPayloadCodec decode pkt.payload.bits,
      bag getPaymentInfo add.paymentHash
    ) match {
      case Attempt.Failure(err: LightningMessageCodecs.MissingRequiredTlv) \ _ =>
        failHtlc(pkt, InvalidOnionPayload(err.tag, 0), add)
      case Attempt.Successful(payload) \ _
          if payload.value.cltvExpiry != add.expiry =>
        failHtlc(pkt, FinalIncorrectCltvExpiry(add.expiry), add)
      case attempt \ Success(info)
          if attempt.isSuccessful && info.pr.msatOrMin > add.amount =>
        failIncorrectDetails(pkt, info.pr.msatOrMin, add)
      case attempt \ Success(info)
          if attempt.isSuccessful && info.pr.msatOrMin * 2 < add.amount =>
        failIncorrectDetails(pkt, info.pr.msatOrMin, add)
      case attempt \ Success(info)
          if LNParams.broadcaster.currentHeight + PaymentRequest.cltvExpiryTag.expiryDelta > add.expiry =>
        failIncorrectDetails(pkt, info.pr.msatOrMin, add)
      case attempt \ Success(info)
          if attempt.isSuccessful && !loop && info.incoming == 1 && info.status != PaymentInfo.SUCCESS =>
        CMDFulfillHtlc(add, info.paymentPreimage)
      case attempt \ _ if attempt.isSuccessful =>
        failIncorrectDetails(pkt, add.amount, add)
      case _ => failHtlc(pkt, InvalidOnionPayload(UInt64(0), 0), add)
    }
}

case class RoutingData(
    pr: PaymentRequest,
    routes: PaymentRouteVec,
    usedRoute: PaymentRoute,
    onion: PacketAndSecrets,
    firstMsat: Long /* amount without off-chain fee */,
    lastMsat: Long /* amount with off-chain fee added */,
    lastExpiry: Long,
    callsLeft: Int,
    useCache: Boolean,
    airLeft: Int,
    capFeeByOnChain: Boolean,
    expensiveScids: Vector[Long],
  ...
  def nextNodeId(route: PaymentRoute) =
    route.headOption.map(_.nodeId) getOrElse pr.nodeId
  lazy val queryText =
    s"${pr.description} ${pr.nodeId.toString} ${pr.paymentHash.toHex}"
...
case class PaymentInfo(
  ...
  lazy val pd = try to[PaymentDescription](description)
  catch {
...
case class UrlAction(domain: Option[String], description: String, url: String)
    extends PaymentAction {
  val finalMessage =
    s"<br>${description take 144}<br><br><font color=#0000FF><tt>$url</tt></font><br>"
...
case class AESAction(
  ...
  val ciphertextBytes = ByteVector
