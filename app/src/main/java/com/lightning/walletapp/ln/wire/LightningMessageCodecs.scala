...
object LightningMessageCodecs { me =>
  ...
  def discriminatorWithDefault[A](discriminator: Codec[A], fallback: Codec[A]) =
    new Codec[A] {
      def encode(element: A) = discriminator encode element recoverWith {
        case _ => fallback encode element
      }
  ...
  def der2wire(signature: ByteVector): ByteVector =
    Crypto decodeSignature signature match {
      case (r, s) =>
        val fixedR = Crypto fixSize ByteVector.view(
          r.toByteArray dropWhile 0.==
        )
        val fixedS = Crypto fixSize ByteVector.view(
  ...
  val scalar = Codec[Scalar](
    ...
    decoder =
      (wire: BitVector) => bytes(32).decode(wire).map(_ map Scalar.apply)
  )
  ...
  val publicKey = Codec[PublicKey](
    encoder =
      (publicKey: PublicKey) => bytes(33).encode(publicKey.value toBin true),
    decoder =
  ...
  val ipv6address: Codec[Inet6Address] = bytes(16).exmap(
    bv =>
      Attempt fromTry Try(Inet6Address.getByAddress(null, bv.toArray, null)),
  ...
  def minimalValue(codec: Codec[UInt64], min: UInt64): Codec[UInt64] =
    codec.exmap(f = {
      case value if value < min =>
        Attempt failure Err("Value is not minimally encoded")
  ...
  val uint64: Codec[UInt64] =
    bytes(8).xmap(UInt64.apply, _.toByteVector padLeft 8)

  val varint: Codec[UInt64] = {
    val large = minimalValue(uint64, 0x100000000L)
    val medium =
      minimalValue(uint32.xmap(UInt64.apply, _.toBigInt.toLong), 0x10000)
    val small =
      minimalValue(uint16.xmap(int => UInt64(int), _.toBigInt.toInt), 0xFD)
    val default: Codec[UInt64] =
  ...
  val varintoverflow: Codec[Long] = varint.narrow(f = {
    case value if value > UInt64(Long.MaxValue) =>
      Attempt failure Err(s"Overflow $value")
    case value => Attempt successful value.toBigInt.toLong
  }, UInt64.apply)

  val bytes32: Codec[ByteVector] = limitedSizeBytes(
    codec = bytesStrict(32).xmap(identity, identity),
    limit = 32
  )
  val zeropaddedstring: Codec[String] =
    fixedSizeBytes(32, utf8).xmap(_.takeWhile(_ != '\u0000'), identity)
  val varsizebinarydataLong: Codec[ByteVector] =
  ...
  def prependmac[A](codec: Codec[A], mac32: Mac32) = Codec[A](
    (a: A) =>
      codec.encode(a).map(bits1 => mac32.mac(bits1.toByteVector).bits ++ bits1),
    (encodedMacBits: BitVector) =>
      (bytes32 withContext "mac").decode(encodedMacBits) match {
        case Attempt.Successful(dr)
            if mac32.verify(dr.value, dr.remainder.toByteVector) =>
          codec.decode(dr.remainder)
        case Attempt.Successful(invalidDr) =>
          Attempt Failure scodec.Err(s"Invalid mac detected: $invalidDr")
  ...
  private val genericTlv = (varint withContext "tag") :: variableSizeBytesLong(
    varintoverflow,
    bytes
  )
  private val genericTlvCodec =
    genericTlv.as[GenericTlv].exmap(validateGenericTlv, validateGenericTlv)

  private def validateGenericTlv(generic: GenericTlv): Attempt[GenericTlv] =
    if (generic.tag.toBigInt % 2 == 0)
  ...
  private def variableSizeUInt64(size: Int, min: Long): Codec[UInt64] =
    minimalValue(
      bytes(size).xmap(f => UInt64(f), _.toByteVector takeRight size),
      min
    )

  val tu64: Codec[UInt64] = discriminated[UInt64]
    .by(uint8)
    .\(0x00) { case value if value < 0x01 => value }(
      variableSizeUInt64(0, 0x00)
    )
    .\(0x01) { case value if value < 0x0100 => value }(
      variableSizeUInt64(1, 0x01)
    )
    .\(0x02) { case value if value < 0x010000 => value }(
      variableSizeUInt64(2, 0x0100)
    )
    .\(0x03) { case value if value < 0x01000000 => value }(
      variableSizeUInt64(3, 0x010000)
    )
    .\(0x04) { case value if value < 0x0100000000L => value }(
      variableSizeUInt64(4, 0x01000000)
    )
    .\(0x05) { case value if value < 0x010000000000L => value }(
      variableSizeUInt64(5, 0x0100000000L)
    )
    .\(0x06) { case value if value < 0x01000000000000L => value }(
      variableSizeUInt64(6, 0x010000000000L)
    )
    .\(0x07) { case value if value < 0x0100000000000000L => value }(
      variableSizeUInt64(7, 0x01000000000000L)
    )
    .\(0x08) { case value if value <= UInt64.MaxValue => value }(
      variableSizeUInt64(8, 0x0100000000000000L)
    )
  ...
  val tu64Overflow: Codec[Long] = tu64.exmap(
    ...
    l =>
      ...
      else Attempt Failure Err(s"uint64 must be positive, actual is $l")
  )
  ...
  def tlvStream[T <: Tlv](
    ...
    list(withFallback).exmap(
      recordsEitherTlvList => {
        ...
        val unknownTags =
          for (Left(unknown) <- recordsEitherTlvList) yield unknown

        val tags = for (record <- recordsEitherTlvList) yield record match {
          case Right(tlv) =>
            codec.encode(tlv).flatMap(varint.decode).require.value
        ...
        if (tags.length != tags.distinct.length)
          ...
          Attempt Failure Err(
            "Tlv records must be ordered by monotonically-increasing types"
          )
      ...
      stream => {
        ...
        if (tags.length != tags.distinct.length)
          ...
          Attempt Successful tags
            .zip(records)
            .sortBy { case (tag, _) => tag }
  ...
  private val amountToForward: Codec[OnionTlv.AmountToForward] =
    (tu64Overflow withContext "amount_msat")
      .xmap(OnionTlv.AmountToForward, _.amountMsat)
  ...
  val relayPerHopPayloadCodec: Codec[RelayPayload] =
    fallback(tlvPerHopPayloadCodec, legacyRelayPerHopPayloadCodec).narrow(
      f = {
        case Left(tlvs) if tlvs.get[OnionTlv.OutgoingChannelId].isEmpty =>
          Attempt failure MissingRequiredTlv(6)
        case Left(tlvs) if tlvs.get[OnionTlv.AmountToForward].isEmpty =>
          Attempt failure MissingRequiredTlv(2)
        case Left(tlvs) if tlvs.get[OnionTlv.OutgoingCltv].isEmpty =>
          Attempt failure MissingRequiredTlv(4)
  ...
  val lightningMessageCodec =
    discriminated[LightningMessage]
      ...
      .typecase(
        cr = updateFailMalformedHtlc.as[UpdateFailMalformedHtlc],
        tag = 135
      )
      ...
      .typecase(
        cr = announcementSignatures.as[AnnouncementSignatures],
        tag = 259
      )
...
case class TlvStream[T <: Tlv](
  ...
  def get[R <: T: ClassTag]: Option[R] = records.collectFirst {
...
sealed trait FinalPayload extends PerHopPayload with PerHopPayloadFormat { me =>
  def encode =
    LightningMessageCodecs.finalPerHopPayloadCodec
      .encode(me)
      .require
      .toByteVector
  val amountMsat: Long
...
sealed trait RelayPayload extends PerHopPayload with PerHopPayloadFormat { me =>
  def encode =
    LightningMessageCodecs.relayPerHopPayloadCodec
      .encode(me)
      .require
      .toByteVector
...
case class FinalLegacyPayload(amountMsat: Long, cltvExpiry: Long)
    extends FinalPayload
    with LegacyFormat
case class RelayLegacyPayload(
    outgoingChannelId: Long,
    amountToForwardMsat: Long,
    outgoingCltv: Long
...
case class RelayTlvPayload(records: OnionTlv.Stream) extends RelayPayload {
  override val outgoingChannelId =
    records.get[OnionTlv.OutgoingChannelId].get.shortChannelId
  override val amountToForwardMsat =
    records.get[OnionTlv.AmountToForward].get.amountMsat
