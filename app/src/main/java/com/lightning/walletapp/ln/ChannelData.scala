...
case class CMDOpenChannel(
    localParams: LocalParams,
    tempChanId: ByteVector,
    initialFeeratePerKw: Long,
    batch: Batch,
    fundingSat: Long,
    channelFlags: ChannelFlags = ChannelFlags(0),
    pushMsat: Long = 0L
) extends Command

case class CMDFailMalformedHtlc(id: Long, onionHash: ByteVector, code: Int)
    extends Command
case class CMDFulfillHtlc(add: UpdateAddHtlc, preimage: ByteVector)
    extends Command
...
sealed trait HasNormalCommits extends ChannelData {
  val commitments: NormalCommits
}
...
case class WaitRemoteHostedReply(
  ...
  require(
    Helpers isValidFinalScriptPubkey refundScriptPubKey,
    "Invalid refundScriptPubKey when opening a hosted channel"
  )
  lazy val invokeMsg =
    InvokeHostedChannel(LNParams.chainHash, refundScriptPubKey, secret)
}
...
case class WaitFundingCreatedRemote(
    announce: NodeAnnouncement,
    localParams: LocalParams,
    remoteParams: AcceptChannel,
    open: OpenChannel
) extends ChannelData
...
case class WaitFundingSignedCore(
    localParams: LocalParams,
    channelId: ByteVector,
    channelFlags: Option[ChannelFlags],
    remoteParams: AcceptChannel,
  ...
  def makeCommitments(signedLocalCommitTx: CommitTx) =
    NormalCommits(
      localParams,
      remoteParams,
      LocalCommit(index = 0L, localSpec, Nil, signedLocalCommitTx),
      remoteCommit,
      localChanges = Changes(Vector.empty, Vector.empty, Vector.empty),
      remoteChanges = Changes(Vector.empty, Vector.empty, Vector.empty),
      localNextHtlcId = 0L,
      remoteNextHtlcId = 0L,
      remoteNextCommitInfo = Right(Tools.randomPrivKey.toPoint),
      signedLocalCommitTx.input,
      ShaHashesWithIndex(Map.empty, None),
      channelId,
      updateOpt = None,
      channelFlags,
...
case class WaitFundingSignedData(
    announce: NodeAnnouncement,
    core: WaitFundingSignedCore,
...
case class WaitBroadcastRemoteData(
    announce: NodeAnnouncement,
    core: WaitFundingSignedCore,
    commitments: NormalCommits,
    their: Option[FundingLocked] = None,
    fundingError: Option[String] = None
) extends HasNormalCommits {

  def isLost: Boolean = fundingError match {
    case None =>
      commitments.startedAt < System.currentTimeMillis - 3600 * 24 * 21 * 1000L
    case _ =>
      commitments.startedAt < System.currentTimeMillis - 3600 * 24 * 7 * 1000L
  }
}

case class WaitFundingDoneData(
    announce: NodeAnnouncement,
    our: Option[FundingLocked],
    their: Option[FundingLocked],
    fundingTx: Transaction,
    commitments: NormalCommits
) extends HasNormalCommits

case class NormalData(
    announce: NodeAnnouncement,
    commitments: NormalCommits,
    unknownSpend: Option[Transaction] = None,
...
case class NegotiationsData(
    announce: NodeAnnouncement,
    commitments: NormalCommits,
    localShutdown: Shutdown,
    remoteShutdown: Shutdown,
    localProposals: Seq[ClosingTxProposed],
    lastSignedTx: Option[ClosingTx] = None
) extends HasNormalCommits

case class RefundingData(
    announce: NodeAnnouncement,
    remoteLatestPoint: Option[Point],
    commitments: NormalCommits
) extends HasNormalCommits

case class ClosingData(
    announce: NodeAnnouncement,
    commitments: NormalCommits,
    localProposals: Seq[ClosingTxProposed] = Nil,
    mutualClose: Seq[Transaction] = Nil,
    localCommit: Seq[LocalCommitPublished] = Nil,
    remoteCommit: Seq[RemoteCommitPublished] = Nil,
    nextRemoteCommit: Seq[RemoteCommitPublished] = Nil,
    refundRemoteCommit: Seq[RemoteCommitPublished] = Nil,
    revokedCommit: Seq[RevokedCommitPublished] = Nil,
  ...
  def canBeRemoved: Boolean =
    System.currentTimeMillis > closedAt + 1000L * 3600 * 24 * 28
...
case class LocalCommitPublished(
    claimMainDelayed: Seq[ClaimDelayedOutputTx],
    claimHtlcSuccess: Seq[SuccessAndClaim],
    claimHtlcTimeout: Seq[TimeoutAndClaim],
    commitTx: Transaction
) extends CommitPublished {

  override def getState = {
    val success = for (tier1 \ tier2 <- claimHtlcSuccess)
      yield HideReady(tier1.tx) :: csvShowDelayed(tier1, tier2, commitTx) :: Nil
    val timeout =
      for (t1 \ t2 <- claimHtlcTimeout)
        yield HideDelayed(cltv(commitTx, t1.tx), t1.tx) :: csvShowDelayed(
          t1,
          t2,
          commitTx
        ) :: Nil
    val main =
      for (t1 <- claimMainDelayed)
        yield ShowDelayed(
          csv(commitTx, t1.tx),
          t1.tx,
          commitTx,
          t1 -- t1,
          t1.tx.allOutputsAmount
        ) :: Nil
...
case class RemoteCommitPublished(
    claimMain: Seq[ClaimP2WPKHOutputTx],
    claimHtlcSuccess: Seq[ClaimHtlcSuccessTx],
    claimHtlcTimeout: Seq[ClaimHtlcTimeoutTx],
    commitTx: Transaction
) extends CommitPublished {

  override def getState = {
    val timeout =
      for (t1 <- claimHtlcTimeout)
        yield ShowDelayed(
          cltv(commitTx, t1.tx),
          t1.tx,
          commitTx,
    ...
    val success =
      for (tier1 <- claimHtlcSuccess)
        yield ShowReady(tier1.tx, tier1 -- tier1, tier1.tx.allOutputsAmount)
    val main =
      for (t1 <- claimMain)
        yield ShowReady(t1.tx, t1 -- t1, t1.tx.allOutputsAmount)
...
case class RevokedCommitPublished(
    claimMain: Seq[ClaimP2WPKHOutputTx],
    claimTheirMainPenalty: Seq[MainPenaltyTx],
    htlcPenalty: Seq[HtlcPenaltyTx],
    commitTx: Transaction
) extends CommitPublished {
...
case class RevocationInfo(
    redeemScriptsToSigs: List[RedeemScriptAndSig],
    claimMainTxSig: Option[ByteVector],
    claimPenaltyTxSig: Option[ByteVector],
    feeRate: Long,
    dustLimit: Long,
  ...
  def makeHtlcPenalty(
      finder: PubKeyScriptIndexFinder
  )(redeemScript: ByteVector) =
    Scripts.makeHtlcPenaltyTx(
      finder,
      redeemScript,
      finalScriptPubKey,
      feeRate,
...
case class CommitmentSpec(
    ...
    toLocalMsat: Long,
    toRemoteMsat: Long,
    htlcs: Set[Htlc] = Set.empty,
    fulfilled: Set[HtlcAndFulfill] = Set.empty,
    failed: Set[HtlcAndFail] = Set.empty,
    malformed: Set[Htlc] = Set.empty
) {

  lazy val fulfilledIncoming = fulfilled collect {
    case Htlc(true, add) \ _ => add
  }
  lazy val fulfilledOutgoing = fulfilled collect {
    case Htlc(false, add) \ _ => add
  }
...
object CommitmentSpec {
  ...
  def fulfill(cs: CommitmentSpec, isIncoming: Boolean, m: UpdateFulfillHtlc) =
    findHtlcById(cs, m.id, isIncoming) match {
      case Some(h) if h.incoming =>
        cs.copy(
          toLocalMsat = cs.toLocalMsat + h.add.amountMsat,
          fulfilled = cs.fulfilled + Tuple2(h, m),
          htlcs = cs.htlcs - h
        )
      case Some(h) =>
        cs.copy(
          toRemoteMsat = cs.toRemoteMsat + h.add.amountMsat,
          fulfilled = cs.fulfilled + Tuple2(h, m),
          htlcs = cs.htlcs - h
        )
      case None => cs
  ...
  def fail(cs: CommitmentSpec, isIncoming: Boolean, m: UpdateFailHtlc) =
    findHtlcById(cs, m.id, isIncoming) match {
      case Some(h) if h.incoming =>
        cs.copy(
          toRemoteMsat = cs.toRemoteMsat + h.add.amountMsat,
          failed = cs.failed + Tuple2(h, m),
          htlcs = cs.htlcs - h
        )
      case Some(h) =>
        cs.copy(
  ...
  def failMalformed(
      cs: CommitmentSpec,
      isIncoming: Boolean,
      m: UpdateFailMalformedHtlc
  ) = findHtlcById(cs, m.id, isIncoming) match {
    case Some(h) if h.incoming =>
      cs.copy(
        toRemoteMsat = cs.toRemoteMsat + h.add.amountMsat,
        malformed = cs.malformed + h,
        htlcs = cs.htlcs - h
      )
    case Some(h) =>
      cs.copy(
  ...
  def plusOutgoing(data: UpdateAddHtlc, cs: CommitmentSpec) =
    cs.copy(
      htlcs = cs.htlcs + Htlc(incoming = false, add = data),
      toLocalMsat = cs.toLocalMsat - data.amountMsat
    )
  def plusIncoming(data: UpdateAddHtlc, cs: CommitmentSpec) =
    cs.copy(
      htlcs = cs.htlcs + Htlc(incoming = true, add = data),
      toRemoteMsat = cs.toRemoteMsat - data.amountMsat
    )

  def reduce(
      cs: CommitmentSpec,
      local: LNMessageVector,
      remote: LNMessageVector
  ) = {
    val spec1 =
      cs.copy(fulfilled = Set.empty, failed = Set.empty, malformed = Set.empty)
    val spec2 = (spec1 /: local) {
      case (s, add: UpdateAddHtlc) => plusOutgoing(add, s)
      case s \ _                   => s
    }
    val spec3 = (spec2 /: remote) {
      case (s, add: UpdateAddHtlc) => plusIncoming(add, s)
      case s \ _                   => s
    }
    ...
    val spec4 = (spec3 /: local) {
      ...
      case (s, msg: UpdateFailMalformedHtlc) =>
        failMalformed(s, isIncoming = true, msg)
    ...
    (spec4 /: remote) {
      ...
      case (s, msg: UpdateFailMalformedHtlc) =>
        failMalformed(s, isIncoming = false, msg)
...
case class WaitingForRevocation(
    ...
    sent: CommitSig,
    localCommitIndexSnapshot: Long
)
case class LocalCommit(
    index: Long,
    spec: CommitmentSpec,
    htlcTxsAndSigs: Seq[HtlcTxAndSigs],
    commitTx: CommitTx
)
...
case class HtlcTxAndSigs(
    ...
    localSig: ByteVector,
    remoteSig: ByteVector
)
case class Changes(
    proposed: LNMessageVector,
    signed: LNMessageVector,
    acked: LNMessageVector
)
...
case class NormalCommits(
  ...
  lazy val reducedRemoteState = {
    ...
    val commitFeeSat =
      Scripts.commitTxFee(remoteParams.dustLimitSat, reduced).toLong
  ...
  def latestRemoteCommit =
    remoteNextCommitInfo.left.toOption
      .map(_.nextRemoteCommit) getOrElse remoteCommit
  def localHasUnsignedOutgoing =
    localChanges.proposed.collectFirst { case msg: UpdateAddHtlc => msg }.isDefined
  def remoteHasUnsignedOutgoing =
    remoteChanges.proposed.collectFirst { case msg: UpdateAddHtlc => msg }.isDefined
  def addRemoteProposal(update: LightningMessage) =
    me.modify(_.remoteChanges.proposed).using(_ :+ update)
  def addLocalProposal(update: LightningMessage) =
    me.modify(_.localChanges.proposed).using(_ :+ update)
  def nextDummyReduced =
    addLocalProposal(Tools.nextDummyHtlc).reducedRemoteState

  def getHtlcCrossSigned(incomingRelativeToLocal: Boolean, htlcId: Long) =
    for {
      _ <- CommitmentSpec.findHtlcById(
        latestRemoteCommit.spec,
        htlcId,
        !incomingRelativeToLocal
      )
      htlcOut <- CommitmentSpec.findHtlcById(
        localSpec,
        htlcId,
        incomingRelativeToLocal
      )
  ...
  def ensureSenderCanAffordChange = {
    ...
    val feesSat =
      if (localParams.isFunder) 0L
      else Scripts.commitTxFee(localParams.dustLimit, reduced).amount
    if (reduced.toRemoteMsat - (feesSat + localParams.channelReserveSat) * 1000L < 0L)
  ...
  def sendAdd(rd: RoutingData) = {
    ...
    val outHtlcs \ inFlight =
      c1.reducedRemoteState.spec.directedHtlcsAndSum(incoming = true)
    if (c1.reducedRemoteState.canSendMsat < 0L)
      throw CMDAddImpossible(rd, ERR_LOCAL_AMOUNT_HIGH)
    if (rd.firstMsat < remoteParams.htlcMinimumMsat)
      throw CMDAddImpossible(
        rd,
        ERR_REMOTE_AMOUNT_LOW,
        remoteParams.htlcMinimumMsat
      )
    if (!c1.localParams.isFunder && c1.reducedRemoteState.canReceiveMsat < 0L)
      throw CMDAddImpossible(rd, ERR_REMOTE_AMOUNT_HIGH)
    if (UInt64(inFlight) > remoteParams.maxHtlcValueInFlightMsat)
      throw CMDAddImpossible(rd, ERR_REMOTE_AMOUNT_HIGH)
    if (outHtlcs.size > remoteParams.maxAcceptedHtlcs)
      throw CMDAddImpossible(rd, ERR_TOO_MANY_HTLC)
    c1 -> add
  }
  ...
  def receiveAdd(add: UpdateAddHtlc) = {
    ...
    val c1 \ reduced = addRemoteProposal(add)
      .modify(_.remoteNextHtlcId)
      .using(_ + 1)
      .ensureSenderCanAffordChange
    ...
    if (inHtlcs.size > localParams.maxAcceptedHtlcs)
      throw new LightningException
    if (UInt64(inFlight) > localParams.maxHtlcValueInFlightMsat)
      throw new LightningException
    c1
  }
  ...
  def receiveFulfill(fulfill: UpdateFulfillHtlc) =
    getHtlcCrossSigned(incomingRelativeToLocal = false, fulfill.id) match {
      case Some(add) if fulfill.paymentHash == add.paymentHash =>
        addRemoteProposal(fulfill)
      case None =>
        throw new LightningException(
          "Peer has fulfilled a non-cross-signed payment"
        )
    }
  ...
  def sendFulfill(cmd: CMDFulfillHtlc) = {
    ...
    val notFound =
      getHtlcCrossSigned(incomingRelativeToLocal = true, cmd.add.id).isEmpty
    if (notFound) throw new LightningException
    else addLocalProposal(fulfill) -> fulfill
  }

  def sendFail(cmd: CMDFailHtlc) = {
    val fail = UpdateFailHtlc(channelId, cmd.id, cmd.reason)
    val notFound =
      getHtlcCrossSigned(incomingRelativeToLocal = true, cmd.id).isEmpty
    if (notFound) throw new LightningException
    else addLocalProposal(fail) -> fail
  }

  def sendFailMalformed(cmd: CMDFailMalformedHtlc) = {
    val failMalformed =
      UpdateFailMalformedHtlc(channelId, cmd.id, cmd.onionHash, cmd.code)
    val notFound = getHtlcCrossSigned(
      incomingRelativeToLocal = true,
      htlcId = cmd.id
    ).isEmpty
    if (notFound) throw new LightningException
    else addLocalProposal(failMalformed) -> failMalformed
  }

  def receiveFail(fail: UpdateFailHtlc) = {
    val notFound =
      getHtlcCrossSigned(incomingRelativeToLocal = false, fail.id).isEmpty
  ...
  def sendCommit(remoteNextPerCommitmentPoint: Point) = {
    val htlcKey = Generators.derivePrivKey(
      localParams.htlcKey,
    ...
    val remoteChanges1 =
      remoteChanges.copy(acked = Vector.empty, signed = remoteChanges.acked)
    val localChanges1 =
      localChanges.copy(proposed = Vector.empty, signed = localChanges.proposed)
    val commitSig = CommitSig(
      channelId,
      Scripts.sign(localParams.fundingPrivKey)(remoteCommitTx),
      htlcSigs.toList
    )
    val remoteCommit1 = RemoteCommit(
      remoteCommit.index + 1,
      spec,
      Some(remoteCommitTx.tx),
      remoteNextPerCommitmentPoint
    )
    val wait = WaitingForRevocation(
      nextRemoteCommit = remoteCommit1,
      commitSig,
      localCommitIndexSnapshot = localCommit.index
    )
    copy(
      remoteNextCommitInfo = Left(wait),
  ...
  def receiveCommit(commit: CommitSig) = {
    ...
    val localPerCommitmentSecret =
      Generators.perCommitSecret(localParams.shaSeed, localCommit.index)
    val localPerCommitmentPoint =
      Generators.perCommitPoint(localParams.shaSeed, localCommit.index + 1)
    val localNextPerCommitmentPoint =
      Generators.perCommitPoint(localParams.shaSeed, localCommit.index + 2)
    val remoteHtlcPubkey = Generators.derivePubKey(
      remoteParams.htlcBasepoint,
      localPerCommitmentPoint
    )
    val localHtlcKey =
      Generators.derivePrivKey(localParams.htlcKey, localPerCommitmentPoint)
    ...
    if (commit.htlcSignatures.size != sortedHtlcTxs.size)
      throw new LightningException
    if (Scripts.checkValid(signedLocalCommitTx).isFailure)
      throw new LightningException
    val htlcSigs =
      for (info <- sortedHtlcTxs) yield Scripts.sign(localHtlcKey)(info)
    val combined =
      (sortedHtlcTxs, htlcSigs, commit.htlcSignatures).zipped.toList

    val htlcTxsAndSigs = combined collect {
      case (htlcTx: HtlcTimeoutTx, localSig, remoteSig) =>
        val check = Scripts checkValid Scripts.addSigs(
          htlcTx,
          localSig,
          remoteSig
        )
    ...
    val remoteChanges1 = remoteChanges.copy(
      proposed = Vector.empty,
      acked = remoteChanges.acked ++ remoteChanges.proposed
    )
    val c1 = copy(
      localChanges = localChanges.copy(acked = Vector.empty),
      remoteChanges = remoteChanges1,
  ...
  def receiveRevocation(rev: RevokeAndAck) = remoteNextCommitInfo match {
    case Left(_)
        if remoteCommit.remotePerCommitmentPoint != rev.perCommitmentSecret.toPoint =>
      throw new LightningException(
        "Peer has supplied a wrong per commitment secret"
      )

    case Left(wait) =>
      val nextIndex = ShaChain.largestTxIndex - remoteCommit.index
      val secrets1 = ShaChain.addHash(
        remotePerCommitmentSecrets,
        rev.perCommitmentSecret.toBin.toArray,
        nextIndex
      )
      val localChanges1 = localChanges.copy(
        signed = Vector.empty,
      ...
      copy(
        localChanges = localChanges1,
        remoteChanges = remoteChanges1,
        remoteCommit = wait.nextRemoteCommit,
        remoteNextCommitInfo = Right(rev.nextPerCommitmentPoint),
...
case class HostedCommits(
    ...
    futureUpdates: Vector[LNDirectionalMessage],
    localSpec: CommitmentSpec,
    updateOpt: Option[ChannelUpdate],
  ...
  lazy val Tuple4(
    ...
    (Tuple4(
      Vector.empty[LightningMessage],
      Vector.empty[LightningMessage],
      lastCrossSignedState.localUpdates,
      lastCrossSignedState.remoteUpdates
    ) /: futureUpdates) {
      case (localMessages, remoteMessages, totalLocalNumber, totalRemoteNumber) \ (msg \ true) =>
        (
          localMessages :+ msg,
          remoteMessages,
          totalLocalNumber + 1,
      ...
      case (localMessages, remoteMessages, totalLocalNumber, totalRemoteNumber) \ (msg \ false) =>
        (
          localMessages,
          remoteMessages :+ msg,
          totalLocalNumber,
  ...
  lazy val invokeMsg = InvokeHostedChannel(
    chainHash,
    lastCrossSignedState.refundScriptPubKey,
    ByteVector.empty
  )
  lazy val nextLocalSpec =
  ...
  def nextLocalUnsignedLCSS(blockDay: Long) = {
    val incomingHtlcs \ outgoingHtlcs =
      nextLocalSpec.htlcs.toList.partition(_.incoming)
    LastCrossSignedState(
      lastCrossSignedState.refundScriptPubKey,
      lastCrossSignedState.initHostedChannel,
      blockDay,
      nextLocalSpec.toLocalMsat,
      nextLocalSpec.toRemoteMsat,
      nextTotalLocal,
      nextTotalRemote,
      incomingHtlcs = incomingHtlcs.map(_.add),
      outgoingHtlcs = outgoingHtlcs.map(_.add),
      localSigOfRemote = ByteVector.empty,
      remoteSigOfLocal = ByteVector.empty
    )
  }

  def findState(remoteLCSS: LastCrossSignedState) =
    for {
  ...
  def addProposal(update: LNDirectionalMessage) =
    copy(futureUpdates = futureUpdates :+ update)
  def newLocalBalanceMsat(so: StateOverride) =
    lastCrossSignedState.initHostedChannel.channelCapacityMsat - so.localBalanceMsat
  def hostedState =
    HostedState(
      channelId,
      nextLocalUpdates,
  ...
  def sendAdd(rd: RoutingData) = {
    ...
    val inHtlcs \ inFlight =
      commits1.nextLocalSpec.directedHtlcsAndSum(incoming = false)
    if (commits1.nextLocalSpec.toLocalMsat < 0L)
      throw CMDAddImpossible(rd, ERR_REMOTE_AMOUNT_HIGH)
    if (rd.firstMsat < lastCrossSignedState.initHostedChannel.htlcMinimumMsat)
      throw CMDAddImpossible(
        rd,
        ERR_REMOTE_AMOUNT_LOW,
        lastCrossSignedState.initHostedChannel.htlcMinimumMsat
      )
    if (UInt64(inFlight) > lastCrossSignedState.initHostedChannel.maxHtlcValueInFlightMsat)
      throw CMDAddImpossible(rd, ERR_REMOTE_AMOUNT_HIGH)
    if (inHtlcs.size > lastCrossSignedState.initHostedChannel.maxAcceptedHtlcs)
      throw CMDAddImpossible(rd, ERR_TOO_MANY_HTLC)
    commits1 -> add
  }
  ...
  def receiveAdd(add: UpdateAddHtlc) = {
    ...
    val inHtlcs \ inFlight =
      commits1.nextLocalSpec.directedHtlcsAndSum(incoming = true)
    if (inHtlcs.size > lastCrossSignedState.initHostedChannel.maxAcceptedHtlcs)
      throw new LightningException
    if (UInt64(inFlight) > lastCrossSignedState.initHostedChannel.maxHtlcValueInFlightMsat)
      throw new LightningException
  ...
  def receiveFulfill(fulfill: UpdateFulfillHtlc) =
    CommitmentSpec.findHtlcById(localSpec, fulfill.id, isIncoming = false) match {
      case Some(htlc) if fulfill.paymentHash == htlc.add.paymentHash =>
        addProposal(fulfill.remote)
      case None =>
        throw new LightningException(
          "Peer has fulfilled a non-existing payment"
        )
    }

  def receiveFail(fail: UpdateFailHtlc) = {
    val notFound = CommitmentSpec
      .findHtlcById(localSpec, fail.id, isIncoming = false)
      .isEmpty
