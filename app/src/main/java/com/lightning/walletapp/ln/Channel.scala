...
import com.lightning.walletapp.ln.crypto.{
  Generators,
  ShaChain,
  ShaHashesWithIndex
}
...
object Channel {
  ...
  def isOpeningOrOperational(chan: Channel) =
    isOperational(chan) || isOpening(chan)
...
abstract class Channel(val isHosted: Boolean)
    extends StateMachine[ChannelData] { me =>
  implicit val context
      : ExecutionContextExecutor = ExecutionContext fromExecutor Executors.newSingleThreadExecutor
  def process(change: Any) = Future(me doProcess change) onFailure {
    case failure => events onException me -> failure
  }
  ...
  def RESOLVE(prevUpds: Traversable[LightningMessage], cs: Commitments) = {
    ...
    val resolves = prevUpds collect {
      case remoteAdd: UpdateAddHtlc =>
        // We don't want to receive a payment into a channel we have originally sent it from in an attempt to rebalance
        val isLoop = cs.localSpec.htlcs.exists(
          htlc =>
            !htlc.incoming && htlc.add.paymentHash == remoteAdd.paymentHash
        )
  ...
  val events: ChannelListener = new ChannelListener {
    override def onProcessSuccess = {
      case ps =>
        for (lst <- listeners if lst.onProcessSuccess isDefinedAt ps)
          lst onProcessSuccess ps
    }
    override def onException = {
      case failure =>
        for (lst <- listeners if lst.onException isDefinedAt failure)
          lst onException failure
    }
    override def onBecome = {
      case transition =>
        for (lst <- listeners if lst.onBecome isDefinedAt transition)
          lst onBecome transition
    }
    override def unknownHostedHtlcsDetected(hc: HostedCommits) =
      for (lst <- listeners) lst unknownHostedHtlcsDetected hc
    override def fulfillReceived(upd: UpdateFulfillHtlc) =
      for (lst <- listeners) lst fulfillReceived upd
    override def outPaymentAccepted(rd: RoutingData) =
      for (lst <- listeners) lst outPaymentAccepted rd
    override def onSettled(cs: Commitments) =
      for (lst <- listeners) lst.onSettled(cs)
  }
}

abstract class NormalChannel extends Channel(isHosted = false) { me =>
  def fundTxId = data match {
    case hnc: HasNormalCommits => hnc.commitments.commitInput.outPoint.txid
    case _                     => ByteVector.empty
  }
  def inFlightHtlcs: Set[Htlc] = data match {
    case hnc: HasNormalCommits => hnc.commitments.reducedRemoteState.spec.htlcs
    case _                     => Set.empty
  }
  def estCanSendMsat = data match {
    case hnc: HasNormalCommits =>
      hnc.commitments.nextDummyReduced.canSendMsat + LNParams.minCapacityMsat
    case _ => 0L
  }
  def estCanReceiveMsat = data match {
    case hnc: HasNormalCommits =>
      hnc.commitments.nextDummyReduced.canReceiveMsat
    case _ => 0L
  }
  def refundableMsat = data match {
    case hnc: HasNormalCommits => hnc.commitments.localCommit.spec.toLocalMsat
    case _                     => 0L
  }
  ...
  def doProcess(change: Any) = {
    Tuple3(data, change, state) match {
      case (InitData(announce), cmd: CMDOpenChannel, WAIT_FOR_INIT) =>
        BECOME(WaitAcceptData(announce, cmd), WAIT_FOR_ACCEPT) SEND OpenChannel(
          LNParams.chainHash,
          cmd.tempChanId,
          cmd.fundingSat,
          cmd.pushMsat,
          cmd.localParams.dustLimit.amount,
          cmd.localParams.maxHtlcValueInFlightMsat,
          cmd.localParams.channelReserveSat,
          htlcMinimumMsat = 1L,
          cmd.initialFeeratePerKw,
          cmd.localParams.toSelfDelay,
          cmd.localParams.maxAcceptedHtlcs,
          cmd.localParams.fundingPrivKey.publicKey,
          cmd.localParams.revocationBasepoint,
          cmd.localParams.paymentBasepoint,
          cmd.localParams.delayedPaymentBasepoint,
          cmd.localParams.htlcBasepoint,
          Generators.perCommitPoint(cmd.localParams.shaSeed, index = 0L),
          cmd.channelFlags
        )

      case (
          InitData(announce),
          Tuple2(localParams: LocalParams, open: OpenChannel),
          WAIT_FOR_INIT
          ) =>
        if (LNParams.chainHash != open.chainHash)
          throw new LightningException("They have provided a wrong chain hash")
        if (open.channelFlags.isPublic)
          throw new LightningException(
            "They are offering a public channel and we only support private ones"
          )
        if (open.pushMsat > 1000L * open.fundingSatoshis)
          throw new LightningException(
            "They are trying to push more than proposed capacity"
          )
        if (open.dustLimitSatoshis > open.channelReserveSatoshis)
          throw new LightningException(
            "Their dust limit exceeds their channel reserve"
          )
        if (open.feeratePerKw < LNParams.minFeeratePerKw)
          throw new LightningException(
            "Their proposed opening on-chain fee is too small"
          )
        if (open.toSelfDelay > LNParams.maxToSelfDelay)
          throw new LightningException("Their toSelfDelay is too high")
        if (open.dustLimitSatoshis < 546L)
          throw new LightningException("Their on-chain dust limit is too low")
        if (open.maxAcceptedHtlcs > 483)
          throw new LightningException("They can accept too many payments")
        if (open.pushMsat < 0)
          throw new LightningException("Their pushMsat is negative")

        val toLocalMsat \ toRemoteMsat =
          (open.pushMsat, open.fundingSatoshis * 1000L - open.pushMsat)
        if (toLocalMsat <= open.channelReserveSatoshis * 1000L && toRemoteMsat <= open.channelReserveSatoshis * 1000L)
          throw new LightningException(
            "Both toLocal and toRemote amounts are less than total channel reserve"
          )

        if (open.fundingSatoshis / open.channelReserveSatoshis < LNParams.channelReserveToFundingRatio / 5)
          throw new LightningException(
            "Their imposed channel reserve is too high relative to capacity"
          )

        val remoteParams = AcceptChannel(
          open.temporaryChannelId,
          open.dustLimitSatoshis,
          open.maxHtlcValueInFlightMsat,
          open.channelReserveSatoshis,
          open.htlcMinimumMsat,
          minimumDepth = 6,
          open.toSelfDelay,
          open.maxAcceptedHtlcs,
          open.fundingPubkey,
          open.revocationBasepoint,
          open.paymentBasepoint,
          open.delayedPaymentBasepoint,
          open.htlcBasepoint,
          open.firstPerCommitmentPoint
        )

        val firstPerCommitPoint =
          Generators.perCommitPoint(localParams.shaSeed, 0L)
        val wait =
          WaitFundingCreatedRemote(announce, localParams, remoteParams, open)
        BECOME(wait, WAIT_FOR_FUNDING) SEND AcceptChannel(
          open.temporaryChannelId,
          localParams.dustLimit.amount,
          localParams.maxHtlcValueInFlightMsat,
          localParams.channelReserveSat,
          htlcMinimumMsat = 1L,
          minimumDepth = LNParams.minDepth,
          localParams.toSelfDelay,
          localParams.maxAcceptedHtlcs,
          localParams.fundingPrivKey.publicKey,
          localParams.revocationBasepoint,
          localParams.paymentBasepoint,
          localParams.delayedPaymentBasepoint,
          localParams.htlcBasepoint,
          firstPerCommitPoint
        )

      case (
          WaitAcceptData(announce, cmd),
          accept: AcceptChannel,
          WAIT_FOR_ACCEPT
          ) if accept.temporaryChannelId == cmd.tempChanId =>
        if (accept.dustLimitSatoshis > cmd.localParams.channelReserveSat)
          throw new LightningException(
            "Our channel reserve is less than their dust"
          )
        if (UInt64(100000000L) > accept.maxHtlcValueInFlightMsat)
          throw new LightningException(
            "Their maxHtlcValueInFlightMsat is too low"
          )
        if (accept.channelReserveSatoshis > cmd.fundingSat / 10)
          throw new LightningException("Their proposed reserve is too high")
        if (accept.toSelfDelay > LNParams.maxToSelfDelay)
          throw new LightningException("Their toSelfDelay is too high")
        if (accept.dustLimitSatoshis < 546L)
          throw new LightningException("Their on-chain dust limit is too low")
        if (accept.htlcMinimumMsat > 546000L)
          throw new LightningException("Their htlcMinimumMsat is too high")
        if (accept.maxAcceptedHtlcs > 483)
          throw new LightningException("They can accept too many payments")
        if (accept.maxAcceptedHtlcs < 1)
          throw new LightningException("They can accept too few payments")
        if (accept.minimumDepth > 6L)
          throw new LightningException("Their minimumDepth is too high")
      ...
      case (
          WaitFundingCreatedRemote(announce, localParams, accept, open),
          FundingCreated(_, txid, outIndex, remoteSig),
          WAIT_FOR_FUNDING
          ) =>
        val (localSpec, localCommitTx, remoteSpec, remoteCommitTx) =
          Funding.makeFirstCommitTxs(
            localParams,
        ...
        val signedLocalCommitTx =
          Scripts.addSigs(
            ...
            Scripts.sign(localParams.fundingPrivKey)(localCommitTx),
            remoteSig
          )

        if (Scripts.checkValid(signedLocalCommitTx).isSuccess) {
          val localSigOfRemoteTx =
            Scripts.sign(localParams.fundingPrivKey)(remoteCommitTx)
          val fundingSigned =
            FundingSigned(Tools.toLongId(txid, outIndex), localSigOfRemoteTx)
          val rc = RemoteCommit(
            index = 0L,
            remoteSpec,
            Some(remoteCommitTx.tx),
            open.firstPerCommitmentPoint
          )
          val core = WaitFundingSignedCore(
            localParams,
            fundingSigned.channelId,
            Some(open.channelFlags),
            accept,
            localSpec,
            rc
          )
          BECOME(
            WaitBroadcastRemoteData(
              announce,
              core,
              core makeCommitments signedLocalCommitTx
            ),
            WAIT_FUNDING_DONE
          ) SEND fundingSigned
      ...
      case (
          WaitFundingData(announce, cmd, accept),
          CMDFunding(fundTx),
          WAIT_FOR_FUNDING
          ) =>
        ...
        val firstRemoteCommit = RemoteCommit(
          index = 0L,
          remoteSpec,
          Some(remoteCommitTx.tx),
          accept.firstPerCommitmentPoint
        ...
        val core = WaitFundingSignedCore(
          cmd.localParams,
          longId,
          Some(cmd.channelFlags),
          accept,
          localSpec,
          firstRemoteCommit
        )
        BECOME(
          WaitFundingSignedData(announce, core, localCommitTx, fundTx),
          WAIT_FUNDING_SIGNED
        ) SEND fundingCreated

      // They have signed our first commit, we can broadcast a local funding tx
      case (
          wait: WaitFundingSignedData,
          remote: FundingSigned,
          WAIT_FUNDING_SIGNED
          ) =>
        val localSignature =
          Scripts.sign(wait.core.localParams.fundingPrivKey)(wait.localCommitTx)
        val localKey \ remoteKey = wait.core.localParams.fundingPrivKey.publicKey -> wait.core.remoteParams.fundingPubkey
        val signedLocalCommitTx = Scripts.addSigs(
          wait.localCommitTx,
          localKey,
          remoteKey,
          localSignature,
        ...
        val wait1 = WaitFundingDoneData(
          wait.announce,
          None,
          None,
          wait.fundingTx,
          wait.core makeCommitments signedLocalCommitTx
        )
        if (Scripts.checkValid(signedLocalCommitTx).isSuccess)
          BECOME(wait1, WAIT_FUNDING_DONE)
        else throw new LightningException
      ...
      case (
        ...
        val isZeroConfSpendablePush =
          wait.commitments.channelFlags.exists(_.isZeroConfSpendablePush)
        if (isZeroConfSpendablePush)
          me UPDATA data1 doProcess CMDConfirmed(fundTx)
        else me UPDATA data1
      ...
      case (norm: NormalData, fulfill: UpdateFulfillHtlc, OPEN) =>
        ...
        me UPDATA norm.copy(
          commitments = norm.commitments receiveFulfill fulfill
        )
      ...
      case (norm: NormalData, fail: UpdateFailMalformedHtlc, OPEN) =>
        ...
        me UPDATA norm.copy(
          commitments = norm.commitments receiveFailMalformed fail
        )
      ...
      case (norm: NormalData, rev: RevokeAndAck, OPEN) =>
        me UPDATA norm.copy(
          commitments = norm.commitments receiveRevocation rev
        )
        val noResolves =
          RESOLVE(norm.commitments.remoteChanges.signed, norm.commitments)
        if (noResolves) me STORE data

        me doProcess CMDProceed
        REV(norm.commitments, rev)

      case (norm: NormalData, fee: UpdateFee, OPEN)
      ...
      case (norm: NormalData, CMDFeerate(satPerKw), OPEN)
          if norm.commitments.localParams.isFunder =>
        val shouldUpdate = LNParams.shouldUpdateFee(
          satPerKw,
          norm.commitments.localSpec.feeratePerKw
        )
        if (shouldUpdate) norm.commitments sendFee satPerKw foreach {
          case c1 \ feeUpdateMessage =>
      ...
      case (
          wait: WaitFundingDoneData,
          CMDShutdown(scriptPubKey),
          WAIT_FUNDING_DONE
          ) =>
        val finalScriptPubKey = scriptPubKey getOrElse wait.commitments.localParams.defaultFinalScriptPubKey
        val ourShutdown =
          Shutdown(wait.commitments.channelId, scriptPubKey = finalScriptPubKey)
        val norm =
          NormalData(wait.announce, wait.commitments, None, ourShutdown.some)
        for (msg <- makeFirstFundingLocked(wait) :: ourShutdown :: Nil)
          me SEND msg
      ...
      case (wait: WaitFundingDoneData, remote: Shutdown, WAIT_FUNDING_DONE) =>
        val our = Shutdown(
          wait.commitments.channelId,
          wait.commitments.localParams.defaultFinalScriptPubKey
        )
        val norm = NormalData(
          wait.announce,
          wait.commitments,
      ...
      case (
          norm @ NormalData(announce, commitments, txOpt, our, their),
          CMDShutdown(script),
          OPEN | SLEEPING
          ) =>
        if (commitments.localHasUnsignedOutgoing || our.isDefined || their.isDefined)
          startLocalClose(norm)
        else {
          val our = Shutdown(
            commitments.channelId,
            script getOrElse commitments.localParams.defaultFinalScriptPubKey
          )
          val norm1 = me STORE NormalData(
            announce,
            commitments,
            txOpt,
            our.some,
      ...
      case (
          NormalData(announce, commitments, txOpt, our, None),
          remote: Shutdown,
          OPEN
          ) =>
      ...
      case (
          NormalData(announce, commitments, txOpt, our, their),
          CMDProceed,
          OPEN
          ) if inFlightHtlcs.isEmpty =>
        our -> their match {
          case Some(ourSig) \ Some(theirSig) =>
            if (commitments.localParams.isFunder) {
              ...
              val firstProposed = Closing.makeFirstClosing(
                commitments,
                ourSig.scriptPubKey,
                theirSig.scriptPubKey
              )
              val neg = NegotiationsData(
                announce,
                commitments,
                ourSig,
                theirSig,
              ...
              val neg =
                NegotiationsData(announce, commitments, ourSig, theirSig, Nil)
      ...
      case (some: HasNormalCommits, cr: ChannelReestablish, SLEEPING)
        ...
        val secret = Generators.perCommitSecret(
          some.commitments.localParams.shaSeed,
          cr.nextRemoteRevocationNumber - 1
        )
        if (cr.yourLastPerCommitmentSecret contains secret)
          ASKREFUNDPEER(some, cr.myCurrentPerCommitmentPoint.get)
        else throw new LightningException
      ...
      case (norm: NormalData, cr: ChannelReestablish, SLEEPING) =>
        ...
        val localDelta = norm.commitments.localChanges.proposed collect {
          case _: UpdateAddHtlc => true
        }
        val remoteDelta = norm.commitments.remoteChanges.proposed collect {
          case _: UpdateAddHtlc => true
        }
        val c1 = norm.commitments
          .modifyAll(_.localChanges.proposed, _.remoteChanges.proposed)
          .setTo(Vector.empty)
          .modify(_.remoteNextHtlcId)
          .using(_ - remoteDelta.size)
          .modify(_.localNextHtlcId)
          .using(_ - localDelta.size)

        def maybeResendOurRevocationMessage =
          if (c1.localCommit.index == cr.nextRemoteRevocationNumber + 1) {
            val localPerCommitmentSecret = Generators.perCommitSecret(
              c1.localParams.shaSeed,
              c1.localCommit.index - 1
            )
            val localNextPerCommitmentPoint = Generators.perCommitPoint(
              c1.localParams.shaSeed,
              c1.localCommit.index + 1
            )
            me SEND RevokeAndAck(
              channelId = c1.channelId,
              localPerCommitmentSecret,
              localNextPerCommitmentPoint
            )
          } else if (c1.localCommit.index != cr.nextRemoteRevocationNumber)
            throw new LightningException
        ...
        c1.remoteNextCommitInfo match {
          ...
          case Left(wait)
              if wait.nextRemoteCommit.index == cr.nextLocalCommitmentNumber =>
          ...
          case Left(wait)
              if wait.nextRemoteCommit.index + 1 == cr.nextLocalCommitmentNumber =>
            maybeResendOurRevocationMessage
          case Right(_)
              if c1.remoteCommit.index + 1 == cr.nextLocalCommitmentNumber =>
            maybeResendOurRevocationMessage
      ...
      case (neg: NegotiationsData, _: ChannelReestablish, SLEEPING) =>
        ...
        val lastClosingSignedOpt =
          neg.localProposals.headOption.map(_.localClosingSigned)
        for (msg <- neg.localShutdown +: lastClosingSignedOpt.toVector)
          me SEND msg
      ...
      case (some: HasNormalCommits, newAnn: NodeAnnouncement, SLEEPING)
          if some.announce.nodeId == newAnn.nodeId && Announcements.checkSig(
            newAnn
          ) =>
      ...
      case (
        ...
        BECOME(wait, SLEEPING)
      case (wait: WaitFundingDoneData, CMDSocketOffline, WAIT_FUNDING_DONE) =>
        BECOME(wait, SLEEPING)
      case (negs: NegotiationsData, CMDSocketOffline, NEGOTIATIONS) =>
        BECOME(negs, SLEEPING)
      case (norm: NormalData, CMDSocketOffline, OPEN) => BECOME(norm, SLEEPING)

      // NEGOTIATIONS MODE

      case (
          neg @ NegotiationsData(
            _,
            commitments,
            localShutdown,
            remoteShutdown,
          ...
          ClosingSigned(_, remoteClosingFee, remoteClosingSig),
          NEGOTIATIONS
          ) =>
        val lastLocalFee = localProposals.headOption.map(
          _.localClosingSigned.feeSatoshis
        ) getOrElse {
          // If we are fundee and we were waiting for them to send their first proposal, we don't have a lastLocalClosingFee
          val closing = Closing.makeFirstClosing(
            commitments,
        ...
        val signedClose =
          Scripts.addSigs(
            closing,
            commitments.localParams.fundingPrivKey.publicKey,
        ...
        Scripts.checkValid(txWithInputInfo = signedClose) match {
          ...
          case Success(okClose) =>
            ...
            val nextProposed = Closing.makeClosing(
              commitments,
              nextCloseFee,
              localShutdown.scriptPubKey,
              remoteShutdown.scriptPubKey
            )
            if (remoteClosingFee == nextCloseFee.amount)
              startMutualClose(neg, okClose.tx) SEND nextProposed.localClosingSigned
            else {
              val d1 = me STORE neg.copy(
                lastSignedTx = Some(okClose),
                localProposals = nextProposed +: neg.localProposals
              )
          ...
          case _ =>
            ...
            throw new LightningException(
              "Remote shutdown signature check failed"
            )
        }
      ...
      case (
          RefundingData(announce, Some(remoteLatestPoint), commitments),
          CMDSpent(spendTx),
          REFUNDING
          )
          // GUARD: we have got a remote commit which we asked them to spend and we have their point
          if spendTx.txIn.exists(
            input => commitments.commitInput.outPoint == input.outPoint
          ) =>
        val rcp =
          Closing.claimRemoteMainOutput(commitments, remoteLatestPoint, spendTx)
        val d1 =
          ClosingData(announce, commitments, refundRemoteCommit = rcp :: Nil)
      ...
      case (cd: ClosingData, CMDSpent(htlcTx), CLOSING)
        ...
        for {
          revCommitPublished <- cd.revokedCommit
          if revCommitPublished spendsFromRevoked htlcTx
          perCommitmentSecret <- Helpers.Closing.extractCommitmentSecret(
            cd.commitments,
            revCommitPublished.commitTx
          )
          punishTx <- Closing.claimRevokedHtlcTxOutputs(
            commitments = cd.commitments,
            htlcTx,
            perCommitmentSecret
          )
          punishTxj = com.lightning.walletapp.lnutils.ImplicitConversions
            .bitcoinLibTx2bitcoinjTx(punishTx)
      ...
      case (some: HasNormalCommits, CMDSpent(tx), _)
          ...
          if tx.txIn.exists(
            input => some.commitments.commitInput.outPoint == input.outPoint
          ) =>
        val nextRemoteCommitLeft =
          some.commitments.remoteNextCommitInfo.left.map(_.nextRemoteCommit)

        Tuple3(GETREV(some.commitments, tx), nextRemoteCommitLeft, some) match {
          case (_, _, close: ClosingData)
              if close.refundRemoteCommit.nonEmpty =>
            Tools log s"Existing refund $tx"
          case (_, _, close: ClosingData)
              if close.mutualClose.exists(_.txid == tx.txid) =>
            Tools log s"Existing mutual $tx"
          case (_, _, close: ClosingData)
              if close.localCommit.exists(_.commitTx.txid == tx.txid) =>
            Tools log s"Existing local $tx"
          case (_, _, close: ClosingData)
              if close.localProposals.exists(_.unsignedTx.tx.txid == tx.txid) =>
            startMutualClose(close, tx)
          case (_, _, negs: NegotiationsData)
              if negs.localProposals.exists(_.unsignedTx.tx.txid == tx.txid) =>
            startMutualClose(negs, tx)
          case (Some(claim), _, closingData: ClosingData) =>
            me CLOSEANDWATCH closingData
              .modify(_.revokedCommit)
              .using(claim +: _)
          case (Some(claim), _, _) =>
            me CLOSEANDWATCH ClosingData(
              some.announce,
              some.commitments,
              revokedCommit = claim :: Nil
            )
          case (_, Left(nextRemote), _)
              if nextRemote.txOpt.exists(_.txid == tx.txid) =>
            startRemoteNextClose(some, nextRemote)
          case _
              if some.commitments.remoteCommit.txOpt
                .exists(_.txid == tx.txid) =>
            startRemoteCurrentClose(some)
          case _ if some.commitments.localCommit.commitTx.tx.txid == tx.txid =>
      ...
      case (null, wait: WaitFundingDoneData, null) =>
        super.become(wait, SLEEPING)
      case (null, wait: WaitBroadcastRemoteData, null) =>
        super.become(wait, SLEEPING)
  ...
  def makeReestablish(
      some: HasNormalCommits,
      nextLocalCommitmentNumber: Long
  ) = {
    val ShaHashesWithIndex(hashes, lastIndex) =
      some.commitments.remotePerCommitmentSecrets
    val yourLastPerCommitmentSecret = lastIndex
      .map(ShaChain.moves)
      .flatMap(ShaChain getHash hashes)
      .map(ByteVector.view)
    val myCurrentPerCommitmentPoint = Generators.perCommitPoint(
      some.commitments.localParams.shaSeed,
      some.commitments.localCommit.index
    )
    val myCurrentPerCommitmentPointOpt = Some(myCurrentPerCommitmentPoint)

    ChannelReestablish(
      some.commitments.channelId,
      nextLocalCommitmentNumber,
      some.commitments.remoteCommit.index,
      yourLastPerCommitmentSecret.map(Scalar.apply) orElse Some(Zeroes),
      myCurrentPerCommitmentPointOpt
    )
  }

  def makeFirstFundingLocked(some: HasNormalCommits) = {
    val first =
      Generators.perCommitPoint(some.commitments.localParams.shaSeed, 1L)
    FundingLocked(some.commitments.channelId, nextPerCommitmentPoint = first)
  }

  def decideOnFundeeTx(wait: WaitBroadcastRemoteData, fundTx: Transaction) =
    Try {
      val ourFirstCommitmentTransaction: Transaction =
        wait.commitments.localCommit.commitTx.tx
      Transaction.correctlySpends(
        ourFirstCommitmentTransaction,
        Seq(fundTx),
        STANDARD_SCRIPT_VERIFY_FLAGS
      )
    } match {
      case Failure(reason) =>
        wait.modify(_.fundingError) setTo Some(reason.getMessage)
      case _ =>
        WaitFundingDoneData(
          wait.announce,
          None,
  ...
  private def becomeOpen(wait: WaitFundingDoneData, their: FundingLocked) = {
    val c1 = wait.commitments.copy(
      remoteNextCommitInfo = Right apply their.nextPerCommitmentPoint
    )
    BECOME(me STORE NormalData(wait.announce, c1), OPEN)
  }

  private def startMutualClose(some: HasNormalCommits, tx: Transaction) =
    some match {
      case closingData: ClosingData =>
        BECOME(
          me STORE closingData.copy(
            mutualClose = tx +: closingData.mutualClose
          ),
          CLOSING
        )
      case neg: NegotiationsData =>
        BECOME(
          me STORE ClosingData(
            neg.announce,
            neg.commitments,
            neg.localProposals,
            tx :: Nil
      ...
      case _ =>
        BECOME(
          me STORE ClosingData(some.announce, some.commitments, Nil, tx :: Nil),
          CLOSING
        )
    }
  ...
  def startLocalClose(some: HasNormalCommits): Unit =
    ...
    Closing.claimCurrentLocalCommitTxOutputs(some.commitments, LNParams.bag) -> some match {
      case (_, neg: NegotiationsData) if neg.lastSignedTx.isDefined =>
        startMutualClose(neg, neg.lastSignedTx.get.tx)
      case (localClaim, closingData: ClosingData) =>
        me CLOSEANDWATCH closingData.copy(localCommit = localClaim :: Nil)
      case (localClaim, _) =>
        me CLOSEANDWATCH ClosingData(
          some.announce,
          some.commitments,
          localCommit = localClaim :: Nil
        )
    }
  ...
  private def startRemoteCurrentClose(some: HasNormalCommits) =
    ...
    Closing.claimRemoteCommitTxOutputs(
      some.commitments,
      some.commitments.remoteCommit,
      LNParams.bag
    ) -> some match {
      case (remoteClaim, closingData: ClosingData) =>
        me CLOSEANDWATCH closingData.copy(remoteCommit = remoteClaim :: Nil)
      case (remoteClaim, _) =>
        me CLOSEANDWATCH ClosingData(
          some.announce,
          some.commitments,
          remoteCommit = remoteClaim :: Nil
        )
    }

  private def startRemoteNextClose(
      some: HasNormalCommits,
    ...
    Closing.claimRemoteCommitTxOutputs(
      ...
      case (remoteClaim, closingData: ClosingData) =>
        me CLOSEANDWATCH closingData.copy(nextRemoteCommit = remoteClaim :: Nil)
      case (remoteClaim, _) =>
        me CLOSEANDWATCH ClosingData(
          some.announce,
          some.commitments,
          nextRemoteCommit = remoteClaim :: Nil
        )
    }
}
...
abstract class HostedChannel extends Channel(isHosted = true) { me =>
  def estCanSendMsat = data match {
    case hc: HostedCommits => hc.localSpec.toLocalMsat
    case _                 => 0L
  }
  def estCanReceiveMsat = data match {
    case hc: HostedCommits => hc.localSpec.toRemoteMsat
    case _                 => 0L
  }
  def inFlightHtlcs: Set[Htlc] = data match {
    case hc: HostedCommits => hc.currentAndNextInFlight
    case _                 => Set.empty
  }
  def isBlockDayOutOfSync(blockDay: Long): Boolean =
    math.abs(blockDay - LNParams.broadcaster.currentBlockDay) > 1
  ...
  def doProcess(change: Any) = {
    Tuple3(data, change, state) match {
      case (wait: WaitRemoteHostedReply, CMDSocketOnline, WAIT_FOR_INIT) =>
        if (isChainHeightKnown)
          BECOME(wait, WAIT_FOR_ACCEPT) SEND wait.invokeMsg
      ...
      case (
          WaitRemoteHostedReply(announce, refundScriptPubKey, _),
          init: InitHostedChannel,
          WAIT_FOR_ACCEPT
          ) =>
        if (init.liabilityDeadlineBlockdays < LNParams.minHostedLiabilityBlockdays)
          throw new LightningException("Their liability deadline is too low")
        if (init.initialClientBalanceMsat > init.channelCapacityMsat)
          throw new LightningException(
            "Their init balance for us is larger than capacity"
          )
        if (init.minimalOnchainRefundAmountSatoshis > LNParams.minHostedOnChainRefundSat)
          throw new LightningException("Their min refund is too high")
        if (init.channelCapacityMsat < LNParams.minHostedOnChainRefundSat)
          throw new LightningException(
            "Their proposed channel capacity is too low"
          )
        if (UInt64(100000000L) > init.maxHtlcValueInFlightMsat)
          throw new LightningException("Their max value in-flight is too low")
        if (init.htlcMinimumMsat > 546000L)
          throw new LightningException("Their minimal payment size is too high")
        ...
        val localHalfSignedHC =
          restoreCommits(
            LastCrossSignedState(
              refundScriptPubKey,
              init,
              LNParams.broadcaster.currentBlockDay,
              ...
              incomingHtlcs = Nil,
              outgoingHtlcs = Nil,
              localSigOfRemote = ByteVector.empty,
              remoteSigOfLocal = ByteVector.empty
            ).withLocalSigOfRemote(LNParams.nodePrivateKey),
            announce
          )

        val localSU =
          localHalfSignedHC.lastCrossSignedState.stateUpdate(isTerminal = true)
        me UPDATA WaitRemoteHostedStateUpdate(announce, localHalfSignedHC) SEND localSU

      case (
          WaitRemoteHostedStateUpdate(announce, hc),
          remoteSU: StateUpdate,
          WAIT_FOR_ACCEPT
          ) =>
        val localCompleteLCSS = hc.lastCrossSignedState.copy(
        ...
        val isRemoteSigOk =
          localCompleteLCSS.verifyRemoteSig(hc.announce.nodeId)

        if (me isBlockDayOutOfSync remoteSU.blockDay)
          throw new LightningException("Their blockday is wrong")
        if (!isRightRemoteUpdateNumber)
          throw new LightningException("Their remote update number is wrong")
        if (!isRightLocalUpdateNumber)
          throw new LightningException("Their local update number is wrong")
        if (!isRemoteSigOk)
          throw new LightningException("Their signature is wrong")
      ...
      case (
        ...
        val isRemoteSigOk =
          remoteLCSS.reverse.verifyRemoteSig(wait.announce.nodeId)
      ...
      case (hc: HostedCommits, CMDProceed, OPEN) =>
        ...
        val nextHC = hc
          .nextLocalUnsignedLCSS(LNParams.broadcaster.currentBlockDay)
          .withLocalSigOfRemote(LNParams.nodePrivateKey)
      ...
      case (
        ...
        val lcss1 = hc
          .nextLocalUnsignedLCSS(blockDay)
          .copy(remoteSigOfLocal = localSigOfRemoteLCSS)
          .withLocalSigOfRemote(LNParams.nodePrivateKey)
        val hc1 = hc.copy(
          lastCrossSignedState = lcss1,
          localSpec = hc.nextLocalSpec,
          futureUpdates = Vector.empty
        )
        val isRemoteSigOk = lcss1.verifyRemoteSig(hc.announce.nodeId)

        if (remoteUpdates < lcss1.localUpdates)
          me SEND lcss1.stateUpdate(isTerminal = false)
        else if (me isBlockDayOutOfSync blockDay)
      ...
      case (hc: HostedCommits, cmd: CMDFulfillHtlc, OPEN) =>
        ...
        val notFound = CommitmentSpec
          .findHtlcById(hc.localSpec, cmd.add.id, isIncoming = true)
          .isEmpty
        if (notFound) throw new LightningException
        else me UPDATA hc.addProposal(fulfill.local) SEND fulfill

      case (hc: HostedCommits, cmd: CMDFailHtlc, OPEN) =>
        val fail = UpdateFailHtlc(hc.channelId, cmd.id, cmd.reason)
        val notFound = CommitmentSpec
          .findHtlcById(hc.localSpec, cmd.id, isIncoming = true)
          .isEmpty
        if (notFound) throw new LightningException
        else me UPDATA hc.addProposal(fail.local) SEND fail

      case (hc: HostedCommits, cmd: CMDFailMalformedHtlc, OPEN) =>
        val fail =
          UpdateFailMalformedHtlc(hc.channelId, cmd.id, cmd.onionHash, cmd.code)
        val notFound = CommitmentSpec
          .findHtlcById(hc.localSpec, cmd.id, isIncoming = true)
          .isEmpty
        if (notFound) throw new LightningException
      ...
      case (hc: HostedCommits, remoteLCSS: LastCrossSignedState, SLEEPING)
          if isChainHeightKnown =>
        val isRemoteSigOk =
          remoteLCSS.reverse.verifyRemoteSig(hc.announce.nodeId)
        val isLocalSigOk = remoteLCSS.verifyRemoteSig(LNParams.nodePublicKey)
      ...
      case (hc: HostedCommits, newAnn: NodeAnnouncement, SLEEPING | SUSPENDED)
          if hc.announce.nodeId == newAnn.nodeId && Announcements.checkSig(
            newAnn
          ) =>
        // Node was SLEEPING for a long time, do not trigger listeners for this update
      ...
      case (
          hc: HostedCommits,
          CMDHostedStateOverride(remoteOverride),
          SUSPENDED
        ...
        val restoredCompleteLCSS =
          hc.lastCrossSignedState
            .copy(
              ...
              remoteBalanceMsat = remoteOverride.localBalanceMsat,
              localUpdates = remoteOverride.remoteUpdates,
              remoteUpdates = remoteOverride.localUpdates,
              blockDay = remoteOverride.blockDay,
              remoteSigOfLocal = remoteOverride.localSigOfRemoteLCSS
            )
            .withLocalSigOfRemote(LNParams.nodePrivateKey)

        val isRemoteSigOk =
          restoredCompleteLCSS.verifyRemoteSig(hc.announce.nodeId)
        if (!isRemoteSigOk)
          throw new LightningException(
            "Provided remote override signature is wrong"
          )
        if (localBalance < 0)
          throw new LightningException(
            "Provided updated local balance is larger than capacity"
          )
        if (!isRightLocalUpdateNumber)
          throw new LightningException(
            "Provided local update number from remote override is wrong"
          )
        if (!isRightRemoteUpdateNumber)
          throw new LightningException(
            "Provided remote update number from remote override is wrong"
          )
        if (me isBlockDayOutOfSync remoteOverride.blockDay)
          throw new LightningException(
            "Remote override blockday is not acceptable"
          )
        BECOME(me STORE restoreCommits(restoredCompleteLCSS, hc.announce), OPEN) SEND restoredCompleteLCSS
          .stateUpdate(isTerminal = true)
        // They may send a new StateUpdate right after overriding
        waitingUpdate = true

      case (null, wait: WaitRemoteHostedReply, null) =>
        super.become(wait, WAIT_FOR_INIT)
      case (null, hc: HostedCommits, null) if hc.getError.isDefined =>
        super.become(hc, SUSPENDED)
  ...
  def syncAndResend(
      hc: HostedCommits,
      leftovers: Vector[LNDirectionalMessage],
    ...
    val hc1 = hc.copy(futureUpdates = leftovers.filter {
      case _ \ isLocal => isLocal
    }, lastCrossSignedState = lcss, localSpec = spec)
  ...
  def restoreCommits(localLCSS: LastCrossSignedState, ann: NodeAnnouncement) = {
    val inHtlcs =
      for (updateAddHtlc <- localLCSS.incomingHtlcs)
        yield Htlc(incoming = true, updateAddHtlc)
    val outHtlcs =
      for (updateAddHtlc <- localLCSS.outgoingHtlcs)
        yield Htlc(incoming = false, updateAddHtlc)
    val localSpec = CommitmentSpec(
      feeratePerKw = 0L,
      localLCSS.localBalanceMsat,
      localLCSS.remoteBalanceMsat,
      htlcs = (inHtlcs ++ outHtlcs).toSet
    )
    HostedCommits(
      ann,
      localLCSS,
