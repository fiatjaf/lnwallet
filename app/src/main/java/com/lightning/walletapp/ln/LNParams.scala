...
object LNParams {
  ...
  lazy val extendedNodeKey =
    derivePrivateKey(master, hardened(46L) :: hardened(0L) :: Nil)
  lazy val extendedCloudKey =
    derivePrivateKey(master, hardened(92L) :: hardened(0L) :: Nil)
  // HashingKey is used for creating domain-specific identifiers when using "linkable payment" LNUrl
  lazy val hashingKey =
    derivePrivateKey(master, hardened(138L) :: 0L :: Nil).privateKey.toBin
  ...
  def maxAcceptableFee(msat: Long, hops: Int, percent: Long = 100L) =
    25000 * (hops + 1) + msat / percent
  def totalRouteFee(route: PaymentRoute, msat: Long) =
    route.reverse.foldLeft(msat) { case amt \ hop => hop.fee(amt) + amt } - msat
  ...
  def updateFeerate =
    for (chan <- ChannelManager.all)
      chan process CMDFeerate(broadcaster.perKwThreeSat)
  def makeLocalParams(
    ...
    val Seq(revocationSecret, paymentKey, delayedPaymentKey, htlcKey, shaSeed) =
      makeChanKeys(fundKey.publicKey)
    LocalParams(
      UInt64(maxCapacity.amount),
      theirReserve,
      toSelfDelay = 2016,
      maxAcceptedHtlcs = 25,
      fundKey,
      revocationSecret.privateKey,
      paymentKey.privateKey,
      delayedPaymentKey.privateKey,
      htlcKey.privateKey,
      finalScriptPubKey,
      dust,
      sha256(shaSeed.privateKey.toBin),
      isFunder
    )
  }
  ...
  def makeChanKeys(fundKey: PublicKey) = {
    ...
    for (idx <- 1L to 5L)
      yield derivePrivateKey(extendedNodeKey, channelKeyPath :+ idx)
  }

  def makeLinkingKey(domain: String): PrivateKey = {
    val material: ByteVector =
      crypto.Mac32.hmac256(key = hashingKey, message = domain)
    derivePrivateKey(
      chain = hardened(138) +: makeKeyPath(material),
      parent = master
    ).privateKey
  }
...
object ChanErrorCodes {
  ...
  final val ERR_HOSTED_IN_FLIGHT_HTLC_WHILE_RESTORING =
    ByteVector.fromValidHex("0007")
  ...
  def translateTag(error: wire.Error) = error.data take 2 match {
    case ERR_HOSTED_WRONG_BLOCKDAY =>
      new LightningException(app getString err_ln_hosted_wrong_blockday)
    case ERR_HOSTED_WRONG_LOCAL_SIG =>
      new LightningException(app getString err_ln_hosted_wrong_local_sig)
    case ERR_HOSTED_WRONG_REMOTE_SIG =>
      new LightningException(app getString err_ln_hosted_wrong_remote_sig)
    case ERR_HOSTED_TOO_MANY_STATE_UPDATES =>
      new LightningException(app getString err_ln_hosted_too_many_state_updates)
    case ERR_HOSTED_TIMED_OUT_OUTGOING_HTLC =>
      new LightningException(
        app getString err_ln_hosted_timed_out_outgoing_htlc
      )
    case ERR_HOSTED_IN_FLIGHT_HTLC_WHILE_RESTORING =>
      new LightningException(
        app getString err_ln_hosted_in_flight_htlc_while_restoring
      )
    case ERR_HOSTED_CHANNEL_DENIED =>
      new LightningException(app getString err_ln_hosted_channel_denied)
...
trait DelayedPublishStatus extends PublishStatus {
  ...
  override def isPublishable = parent match {
    case pd \ false \ 0L => pd > 0L
    case _               => false
  }
  def delay = parent match {
    case pd \ false \ blocksLeft => blocksLeft
...
case class ShowReady(txn: Transaction, fee: Satoshi, amount: Satoshi)
    extends PublishStatus
case class HideDelayed(parent: (DepthAndDead, Long), txn: Transaction)
    extends DelayedPublishStatus
case class ShowDelayed(
    parent: (DepthAndDead, Long),
    txn: Transaction,
...
trait Broadcaster extends ChannelListener {
  ...
  def csvShowDelayed(
      t1: TransactionWithInputInfo,
      t2: TransactionWithInputInfo,
      commitTx: Transaction
  ) =
    ShowDelayed(
      parent = csv(t1.tx, t2.tx),
      t2.tx,
      commitTx,
      fee = t1 -- t2,
      t2.tx.allOutputsAmount
    )
}
