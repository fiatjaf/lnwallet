...
case class Init(globalFeatures: ByteVector, localFeatures: ByteVector)
...
case class OpenChannel(
    chainHash: ByteVector,
    temporaryChannelId: ByteVector,
...
case class FundingSigned(channelId: ByteVector, signature: ByteVector)
    extends ChannelSetupMessage
...
case class FundingLocked(channelId: ByteVector, nextPerCommitmentPoint: Point)
    extends ChannelMessage { me =>
  def some = Some(me)
}

case class Shutdown(channelId: ByteVector, scriptPubKey: ByteVector)
    extends ChannelMessage { me =>
  def some = Some(me)
}
...
case class UpdateAddHtlc(
    ...
    expiry: Long,
    onionRoutingPacket: OnionRoutingPacket = Sphinx.emptyOnionPacket
) extends ChannelMessage {
...
case class UpdateFailHtlc(channelId: ByteVector, id: Long, reason: ByteVector)
    extends ChannelMessage
case class UpdateFailMalformedHtlc(
    channelId: ByteVector,
    id: Long,
    onionHash: ByteVector,
    failureCode: Int
) extends ChannelMessage
...
case class UpdateFee(channelId: ByteVector, feeratePerKw: Long)
    extends ChannelMessage
case class CommitSig(
    channelId: ByteVector,
    signature: ByteVector,
    htlcSignatures: List[ByteVector] = Nil
) extends ChannelMessage
...
case class Error(channelId: ByteVector, data: ByteVector)
    extends ChannelMessage {
...
case class ChannelReestablish(
    ...
    myCurrentPerCommitmentPoint: Option[Point] = None
) extends ChannelMessage
...
case class ChannelUpdate(
  ...
  require(
    requirement = (messageFlags & 1) != 0 == htlcMaximumMsat.isDefined,
    "htlcMaximumMsat is not consistent with messageFlags value"
  )
  lazy val isHosted = Tools.fromShortId(shortChannelId) match {
    case (blockHeight, _, _) => blockHeight <= LNParams.maxHostedBlockHeight
  }
...
case class Hop(
  ...
  lazy val feeBreakdown =
    f"${feeProportionalMillionths / 10000d}%2f%% of payment sum + baseline $feeBaseMsat msat"
  lazy val humanDetails =
    s"Node ID: $nodeId, Channel ID: $shortChannelId, Expiry: $cltvExpiryDelta blocks, Routing fees: $feeBreakdown"
  def fee(amountMsat: Long) =
    feeBaseMsat + (feeProportionalMillionths * amountMsat) / 1000000L
}
...
case class NodeAnnouncement(
    ...
    rgbColor: RGB,
    alias: String,
    addresses: NodeAddressList,
    unknownFields: ByteVector = ByteVector.empty
) extends RoutingMessage {

  val pretty = addresses collectFirst {
    case _: IPv4 | _: IPv6 =>
      nodeId.toString take 15 grouped 3 mkString "\u0020"
    case _: Tor2 =>
      s"<strong>Tor</strong>\u0020${nodeId.toString take 12 grouped 3 mkString "\u0020"}"
    case _: Tor3 =>
      s"<strong>Tor</strong>\u0020${nodeId.toString take 12 grouped 3 mkString "\u0020"}"
  ...
  val asString =
    s"<strong>${alias.takeWhile(_ != '\u0020') take 18}</strong><br><small>$pretty</small>"
  lazy val hostedChanId =
    Tools.hostedChanId(LNParams.nodePublicKey.toBin, nodeId.toBin)
  def unsafeFirstAddress =
    addresses.collectFirst(NodeAddress.toInetSocketAddress)
}
...
case object NodeAddress {
  ...
  def toInetSocketAddress: PartialFunction[NodeAddress, InetSocketAddress] = {
    case Tor2(onionHost, port) =>
      new InetSocketAddress(s"$onionHost$onionSuffix", port)
    case Tor3(onionHost, port) =>
      new InetSocketAddress(s"$onionHost$onionSuffix", port)
  ...
  def resolveIp(host: String, port: Int) = InetAddress getByName host match {
    case inetVersion4Address: Inet4Address =>
      IPv4(ipv4 = inetVersion4Address, port)
    case inetVersion6Address: Inet6Address =>
      IPv6(ipv6 = inetVersion6Address, port)
  }
}
...
case class InitHostedChannel(
    ...
    htlcMinimumMsat: Long,
    maxAcceptedHtlcs: Int,
    channelCapacityMsat: Long,
    liabilityDeadlineBlockdays: Int,
    minimalOnchainRefundAmountSatoshis: Long,
    initialClientBalanceMsat: Long
) extends HostedChannelMessage
...
case class LastCrossSignedState(
    ...
    blockDay: Long,
    localBalanceMsat: Long,
    remoteBalanceMsat: Long,
    localUpdates: Long,
    remoteUpdates: Long,
    incomingHtlcs: List[UpdateAddHtlc],
    outgoingHtlcs: List[UpdateAddHtlc],
    remoteSigOfLocal: ByteVector,
    localSigOfRemote: ByteVector
) extends HostedChannelMessage { me =>
  ...
  lazy val hostedSigHash: ByteVector = Crypto sha256 {
    val inPayments = incomingHtlcs
      .map(updateAddHtlcCodec.encode(_).require.toByteVector)
      .sortWith(LexicographicalOrdering.isLessThan)
    val outPayments = outgoingHtlcs
      .map(updateAddHtlcCodec.encode(_).require.toByteVector)
      .sortWith(LexicographicalOrdering.isLessThan)

    refundScriptPubKey ++
      Protocol.writeUInt16(
        initHostedChannel.liabilityDeadlineBlockdays,
        LITTLE_ENDIAN
      ) ++
      Protocol.writeUInt64(
        initHostedChannel.minimalOnchainRefundAmountSatoshis,
        LITTLE_ENDIAN
      ) ++
      Protocol.writeUInt64(initHostedChannel.channelCapacityMsat, LITTLE_ENDIAN) ++
      Protocol.writeUInt64(
        initHostedChannel.initialClientBalanceMsat,
        LITTLE_ENDIAN
      ) ++
  ...
  def verifyRemoteSig(pubKey: PublicKey) =
    Crypto.verifySignature(hostedSigHash, remoteSigOfLocal, pubKey)
  def withLocalSigOfRemote(priv: PrivateKey) =
    me.modify(_.localSigOfRemote) setTo sign(reverse.hostedSigHash, priv)

  def isAhead(remoteLCSS: LastCrossSignedState) =
    remoteUpdates > remoteLCSS.localUpdates || localUpdates > remoteLCSS.remoteUpdates
  def isEven(remoteLCSS: LastCrossSignedState) =
    remoteUpdates == remoteLCSS.localUpdates && localUpdates == remoteLCSS.remoteUpdates
  def stateUpdate(isTerminal: Boolean) =
    StateUpdate(
      blockDay,
      localUpdates,
      remoteUpdates,
...
case class StateUpdate(
    ...
    localUpdates: Long,
    remoteUpdates: Long,
    localSigOfRemoteLCSS: ByteVector,
    isTerminal: Boolean
) extends HostedChannelMessage
...
case class LocalBackups(
    normal: Vector[HasNormalCommits],
    hosted: Vector[HostedCommits],
    earliestUtxoSeconds: Long,
    v: Int
)
case class HostedState(
    channelId: ByteVector,
    nextLocalUpdates: LNMessageVector,
    nextRemoteUpdates: LNMessageVector,
