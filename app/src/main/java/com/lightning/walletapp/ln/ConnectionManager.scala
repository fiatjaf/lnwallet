...
object ConnectionManager {
  ...
  protected[this] val events = new ConnectionListener {
    override def onMessage(nodeId: PublicKey, msg: LightningMessage) =
      for (lst <- listeners) lst.onMessage(nodeId, msg)
    override def onHostedMessage(
        ann: NodeAnnouncement,
        msg: HostedChannelMessage
    ) = for (lst <- listeners) lst.onHostedMessage(ann, msg)
    override def onOperational(nodeId: PublicKey, isCompat: Boolean) =
      for (lst <- listeners) lst.onOperational(nodeId, isCompat)
    override def onDisconnect(nodeId: PublicKey) =
      for (lst <- listeners) lst.onDisconnect(nodeId)
  }
  ...
  class Worker(
    ...
    val handler: TransportHandler = new TransportHandler(keyPair, ann.nodeId) {
      def handleEncryptedOutgoingData(data: ByteVector) =
        try sock.getOutputStream write data.toArray
        catch handleError
      def handleDecryptedIncomingData(data: ByteVector) =
        Tuple2(LightningMessageCodecs deserialize data, ourLastPing) match {
          case (init: Init, _) =>
            events.onOperational(
              isCompat = areSupported(init.localFeatures) && dataLossProtect(
                init.localFeatures
              ),
              nodeId = ann.nodeId
            )
          case Ping(replyLength, _) \ _
              if replyLength > 0 && replyLength <= 65532 =>
            handler process Pong(ByteVector fromValidHex "00" * replyLength)
          case Pong(randomData) \ Some(ourPing)
              if randomData.size == ourPing.pongLength =>
            ourLastPing = None
          case (message: HostedChannelMessage, _) =>
            events.onHostedMessage(ann, message)
      ...
      def handleEnterOperationalState = {
        ...
        pinging = Obs
          .interval(15.seconds)
          .map(_ => random.nextInt(10) + 1) subscribe { length =>
          val ourNextPing = Ping(
            data = ByteVector.view(random getBytes length),
            pongLength = length
          )
...
class ConnectionListener {
  ...
  def onHostedMessage(ann: NodeAnnouncement, msg: HostedChannelMessage): Unit =
    none
