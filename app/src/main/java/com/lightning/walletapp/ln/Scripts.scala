...
object Scripts { me =>
  ...
  def witness2of2(
    ...
    LexicographicalOrdering.isLessThan(pubkey1.toBin, pubkey2.toBin) -> multiSig2of2(
      pubkey1,
      pubkey2
    ) match {
      case (false, multisig) =>
        ScriptWitness(
          ByteVector.empty :: sig2 :: sig1 :: Script.write(multisig) :: Nil
        )
      case (true, multisig) =>
        ScriptWitness(
          ByteVector.empty :: sig1 :: sig2 :: Script.write(multisig) :: Nil
        )
    }
  ...
  trait TransactionWithInputInfo {
    ...
    def --(that: TransactionWithInputInfo) =
      input.txOut.amount - that.tx.allOutputsAmount
    def input: InputInfo
    def tx: Transaction
  }
  ...
  case class ClaimP2WPKHOutputTx(input: InputInfo, tx: Transaction)
      extends TransactionWithInputInfo
  case class ClaimDelayedOutputTx(input: InputInfo, tx: Transaction)
      extends TransactionWithInputInfo
  case class ClaimDelayedOutputPenaltyTx(input: InputInfo, tx: Transaction)
      extends TransactionWithInputInfo
  ...
  private def trimOfferedHtlcs(dustLimit: Satoshi, spec: CommitmentSpec) = {
    ...
    spec.htlcs.collect {
      case Htlc(false, add) if add.amount >= htlcTimeoutFee => add
    }.toSeq
  }
  ...
  private def trimReceivedHtlcs(dustLimit: Satoshi, spec: CommitmentSpec) = {
    ...
    spec.htlcs.collect {
      case Htlc(true, add) if add.amount >= htlcSuccessFee => add
    }.toSeq
  }
  ...
  def commitTxFee(dustLimit: Satoshi, spec: CommitmentSpec) = {
    ...
    weight2fee(
      spec.feeratePerKw,
      commitWeight + trimmedOfferedHtlcs + trimmedReceivedHtlcs
    )
  }
  ...
  def obscuredCommitTxNumber(
    ...
    val (paymentBasepoint1, paymentBasepoint2) =
      if (isFunder) (local, remote) else (remote, local)
    val combined = paymentBasepoint1.toBin(compressed = true) ++ paymentBasepoint2
      .toBin(compressed = true)
    val blind = (Crypto sha256 combined takeRight 6).reverse ++ ByteVector
      .fromValidHex("0000")
  ...
  def getCommitTxNumber(
    ...
    decodeTxNumber(commitTx.txIn.head.sequence, commitTx.lockTime) ^ obscuredCommitTxNumber(
      0,
      isFunder,
      local,
      remote
  ...
  def decodeTxNumber(sequence: Long, locktime: Long) =
  ...
  def addSigs(
    ...
    commit
      .modify(_.tx)
      .using(
        _ updateWitnesses witness2of2(localSig, remoteSig, localKey, remoteKey) :: Nil
      )

  def addSigs(
      closing: ClosingTx,
      localFunding: PublicKey,
      remoteFunding: PublicKey,
      localSig: ByteVector,
      remoteSig: ByteVector
  ): ClosingTx =
    closing
  ...
  def addSigs(
    ...
    mainPenaltyTx
      .modify(_.tx)
      .using(
        _ updateWitnesses ScriptWitness(
          revocationSig :: ByteVector(1) ::
            mainPenaltyTx.input.redeemScript :: Nil
        ) :: Nil
      )
  ...
  def addSigs(
    ...
    htlcPenaltyTx
      ...
      .using(
        _ updateWitnesses ScriptWitness(
          revocationSig :: revocationPubkey.toBin ::
            htlcPenaltyTx.input.redeemScript :: Nil
        ) :: Nil
      )
  ...
  def addSigs(
      ...
      localSig: ByteVector,
      remoteSig: ByteVector,
      paymentPreimage: ByteVector
  ): HtlcSuccessTx =
    htlcSuccessTx
      .modify(_.tx)
      .using(
        _ updateWitnesses ScriptWitness(
          ByteVector.empty :: remoteSig :: localSig :: paymentPreimage ::
            htlcSuccessTx.input.redeemScript :: Nil
        ) :: Nil
      )
  ...
  def addSigs(
    ...
    htlcTimeoutTx
      ...
      .using(
        _ updateWitnesses ScriptWitness(
          ByteVector.empty :: remoteSig ::
            localSig :: ByteVector.empty :: htlcTimeoutTx.input.redeemScript :: Nil
        ) :: Nil
      )
  ...
  def addSigs(
    ...
    claimHtlcSuccessTx
      ...
      .using(
        _ updateWitnesses ScriptWitness(
          localSig :: paymentPreimage ::
            claimHtlcSuccessTx.input.redeemScript :: Nil
        ) :: Nil
      )
  ...
  def addSigs(
    ...
    claimHtlcTimeoutTx
      ...
      .using(
        _ updateWitnesses ScriptWitness(
          localSig :: ByteVector.empty ::
            claimHtlcTimeoutTx.input.redeemScript :: Nil
        ) :: Nil
      )
  ...
  def addSigs(
      ...
      localSig: ByteVector,
      localPaymentPubkey: ByteVector
  ): ClaimP2WPKHOutputTx =
    claimP2WPKHOutputTx
      .modify(_.tx)
      .using(
        _ updateWitnesses ScriptWitness(localSig :: localPaymentPubkey :: Nil) :: Nil
      )

  def addSigs(
      claimHtlcDelayed: ClaimDelayedOutputTx,
      localSig: ByteVector
    ...
    claimHtlcDelayed
      ...
      .using(
        _ updateWitnesses ScriptWitness(
          localSig :: ByteVector.empty ::
            claimHtlcDelayed.input.redeemScript :: Nil
        ) :: Nil
      )

  def addSigs(
      claimHtlcDelayedPenalty: ClaimDelayedOutputPenaltyTx,
      revocationSig: ByteVector
  ): ClaimDelayedOutputPenaltyTx =
    claimHtlcDelayedPenalty
      .modify(_.tx)
      .using(
        _ updateWitnesses Scripts.witnessToLocalDelayedWithRevocationSig(
          revocationSig,
  ...
  def checkValid[T <: TransactionWithInputInfo](txWithInputInfo: => T) = Try {
    if (txWithInputInfo.tx.txOut.isEmpty)
      throw new Exception("Empty transaction found")
    val check =
      Map(txWithInputInfo.tx.txIn.head.outPoint -> txWithInputInfo.input.txOut)
    Transaction
      .correctlySpends(txWithInputInfo.tx, check, STANDARD_SCRIPT_VERIFY_FLAGS)
    txWithInputInfo
  }
  ...
  def makeCommitTx(
    ...
    val toRemoteOutput =
      if (remote < localDustLimit) Nil
      else TxOut(remote, Script pay2wpkh remotePaymentPubkey) :: Nil
    val toLocalDelayedOutput =
      if (local < localDustLimit) Nil
      else TxOut(local, Script pay2wsh redeem) :: Nil

    val htlcOfferedOutputs = trimOfferedHtlcs(dustLimit = localDustLimit, spec) map {
      add =>
        val offered = htlcOffered(
          localHtlcPubkey,
          remoteHtlcPubkey,
          localRevocationPubkey,
          add.hash160
    ...
    val htlcReceivedOutputs = trimReceivedHtlcs(
      dustLimit = localDustLimit,
      spec
    ) map { add =>
      val received = htlcReceived(
        localHtlcPubkey,
        remoteHtlcPubkey,
        localRevocationPubkey,
        add.hash160,
    ...
    val sequence \ locktime =
      me encodeTxNumber obscuredCommitTxNumber(
        commitTxNumber,
        localIsFunder,
  ...
  def makeHtlcTxs(
    ...
    def makeHtlcTx(
        redeem: Seq[ScriptElt],
        pubKeyScript: Seq[ScriptElt],
        amount: Satoshi,
        fee: Satoshi,
        expiry: Long
    ) = {
      val index = finder.findPubKeyScriptIndex(
        pubkeyScript = Script.write(Script pay2wsh redeem),
        Option apply amount
      )
      val inputInfo = InputInfo(
        OutPoint(commitTx, index),
        commitTx.txOut(index),
        Script write redeem
      )
    ...
    val htlcTimeoutTxs = trimOfferedHtlcs(dustLimit, spec) map { add =>
      ...
      val pubKeyScript = Script pay2wsh toLocalDelayed(
        localRevPubkey,
        toLocalDelay,
        localDelayedPaymentPubkey
      )
      val info \ tx = makeHtlcTx(
        offered,
        pubKeyScript,
        add.amount,
        weight2fee(spec.feeratePerKw, htlcTimeoutWeight),
        add.expiry
      )
    ...
    val htlcSuccessTxs = trimReceivedHtlcs(dustLimit, spec) map { add =>
      ...
      val info \ tx = makeHtlcTx(
        ...
        pubKeyScript,
        add.amount,
        weight2fee(spec.feeratePerKw, htlcSuccessWeight),
        0L
      )
  ...
  def makeClaimHtlcTimeoutTx(
      ...
      feeratePerKw: Long,
      dustLimit: Satoshi
  ): Try[ClaimHtlcTimeoutTx] = Try {

    val redeem = htlcReceived(
      remoteHtlcPubkey,
      localHtlcPubkey,
    ...
    val index = finder.findPubKeyScriptIndex(
      Script.write(Script pay2wsh redeem),
      Option apply add.amount
    )
    val inputInfo = InputInfo(
      OutPoint(finder.tx, index),
      finder.tx.txOut(index),
      Script write redeem
    )
    val finalAmount = inputInfo.txOut.amount - weight2fee(
      feeratePerKw,
      claimHtlcTimeoutWeight
    )
    if (finalAmount < dustLimit)
  ...
  def makeClaimHtlcSuccessTx(
      ...
      feeratePerKw: Long,
      dustLimit: Satoshi
  ): Try[ClaimHtlcSuccessTx] = Try {

    val redeem = htlcOffered(
      remoteHtlcPubkey,
      localHtlcPubkey,
      remoteRevocationPubkey,
      add.hash160
  ...
  def makeClaimP2WPKHOutputTx(
      ...
      feeratePerKw: Long,
      dustLimit: Satoshi
  ): Try[ClaimP2WPKHOutputTx] = Try {

    val redeem = Script pay2pkh localPaymentPubkey
    val finder = new PubKeyScriptIndexFinder(delayedOutputTx)
    val index = finder.findPubKeyScriptIndex(
      Script.write(Script pay2wpkh localPaymentPubkey),
      None
    )
    val inputInfo = InputInfo(
      OutPoint(delayedOutputTx, index),
      delayedOutputTx.txOut(index),
      Script write redeem
    )
    val finalAmount = inputInfo.txOut.amount - weight2fee(
      feeratePerKw,
  ...
  def makeHtlcPenaltyTx(
    ...
    val inputInfo = InputInfo(
      outPoint = OutPoint(finder.tx, index),
      finder.tx.txOut(index),
      redeem
    )
    val finalAmount = inputInfo.txOut.amount - weight2fee(
  ...
  def makeClaimDelayedOutputPenaltyTx(
      delayedOutputTx: Transaction,
      localRevocationPubkey: PublicKey,
      toLocalDelay: Int,
      localDelayedPaymentPubkey: PublicKey,
      ...
      feeratePerKw: Long,
      dustLimit: Satoshi
  ): Try[ClaimDelayedOutputPenaltyTx] = Try {
...
class PubKeyScriptIndexFinder(val tx: Transaction) {
  ...
  def findPubKeyScriptIndex(
    ...
    val index = indexedOutputs indexWhere {
      case out \ idx =>
