...
sealed trait FailureMessage { me =>
  lazy val code: Int =
    failureMessageCodec.encode(me).flatMap(uint16.decode).require.value
}
...
case class IncorrectOrUnknownPaymentDetails(amountMsat: Long, height: Long)
    extends Perm
...
case class AmountBelowMinimum(amountMsat: Long, update: ChannelUpdate)
    extends Update
...
case class FeeInsufficient(amountMsat: Long, update: ChannelUpdate)
    extends Update
case class IncorrectCltvExpiry(expiry: Long, update: ChannelUpdate)
    extends Update
...
object FailureMessageCodecs {
  ...
  private val channelUpdateCodecWithType =
    lightningMessageCodec.narrow[ChannelUpdate](
      Attempt successful _.asInstanceOf[ChannelUpdate],
      identity
    )
  private val channelUpdateWithLengthCodec = variableSizeBytes(
    value = choice(channelUpdateCodecWithType, channelUpdateCodec),
    size = uint16
  )
  ...
  val failureMessageCodec =
    discriminatorWithDefault(
      discriminated[FailureMessage]
        ...
        .typecase(
          cr = provide(RequiredNodeFeatureMissing),
          tag = PERM | NODE | 3
        )
        .typecase(
          cr = bytes32.as[InvalidOnionVersion],
          tag = BADONION | PERM | 4
        )
        .typecase(cr = bytes32.as[InvalidOnionHmac], tag = BADONION | PERM | 5)
        .typecase(cr = bytes32.as[InvalidOnionKey], tag = BADONION | PERM | 6)
        .typecase(
          cr = channelUpdateWithLengthCodec.as[TemporaryChannelFailure],
          tag = UPDATE | 7
        )
        ...
        .typecase(
          cr = channelUpdateWithLengthCodec.as[ExpiryTooSoon],
          tag = UPDATE | 14
        )
        .typecase(
          cr = incorrectOrUnknownPaymentDetails
            .as[IncorrectOrUnknownPaymentDetails],
          tag = PERM | 15
        )
        .typecase(
          cr = (uint32 withContext "expiry").as[FinalIncorrectCltvExpiry],
          tag = 18
        )
        .typecase(
          cr = (uint64Overflow withContext "amountMsat")
            .as[FinalIncorrectHtlcAmount],
          tag = 19
        )
        .typecase(cr = disabled.as[ChannelDisabled], tag = UPDATE | 20)
        .typecase(cr = provide(ExpiryTooFar), tag = 21)
        .typecase(
          cr = invalidOnionPayload.as[InvalidOnionPayload],
          tag = PERM | 22
        ),
      uint16.xmap(
        unknownFailureFromCode(_).asInstanceOf[FailureMessage],
