...
class LNOpsActivity extends TimerActivity with HumanTimeDisplay { me =>
  ...
  val adapter = new BaseAdapter {
    ...
    def getView(position: Int, savedView: View, parent: ViewGroup) = {
      ...
      val card =
        if (null == savedView)
          getLayoutInflater.inflate(R.layout.chan_card, null)
        else savedView

      val cardView =
        Tuple3(getItem(position), getItem(position).data, card.getTag) match {
          case (
              chan: HostedChannel,
          ...
          case (chan: HostedChannel, commits: HostedCommits, _) =>
            new HostedViewHolder(card).fill(chan, commits)
          case (
              chan: NormalChannel,
              commits: HasNormalCommits,
              view: NormalViewHolder
              ) =>
            view.fill(chan, commits.commitments)
          case (chan: NormalChannel, commits: HasNormalCommits, _) =>
            new NormalViewHolder(card).fill(chan, commits.commitments)
  ...
  val eventsListener = new ChannelListener with BlocksListener {
    override def onBecome = {
      case _ => UITask(adapter.notifyDataSetChanged).run
    }
  ...
  abstract class ChanViewHolder(view: View) {
    ...
    val extraInfoText =
      view.findViewById(R.id.extraInfoText).asInstanceOf[TextView]
    val addressAndKey =
      view.findViewById(R.id.addressAndKey).asInstanceOf[TextView]
    val stateAndConnectivity =
      view.findViewById(R.id.stateAndConnectivity).asInstanceOf[TextView]
    def setExtraInfo(text: CharSequence) =
      wrap(extraInfoText setVisibility View.VISIBLE)(extraInfoText setText text)
    def setExtraInfo(resource: Int) =
      wrap(extraInfoText setVisibility View.VISIBLE)(
        extraInfoText setText resource
      )
    ...
    val totalPaymentsText =
      view.findViewById(R.id.totalPaymentsText).asInstanceOf[TextView]
    val refundableAmountText =
      view.findViewById(R.id.refundableAmountText).asInstanceOf[TextView]
    val paymentsInFlightText =
      view.findViewById(R.id.paymentsInFlightText).asInstanceOf[TextView]
    val totalCapacityText =
      view.findViewById(R.id.totalCapacityText).asInstanceOf[TextView]
    val fundingDepthText =
      view.findViewById(R.id.fundingDepthText).asInstanceOf[TextView]
    val canReceiveText =
      view.findViewById(R.id.canReceiveText).asInstanceOf[TextView]
    val startedAtText =
      view.findViewById(R.id.startedAtText).asInstanceOf[TextView]
    val refundFeeText =
      view.findViewById(R.id.refundFeeText).asInstanceOf[TextView]
    val closedAtText =
      view.findViewById(R.id.closedAtText).asInstanceOf[TextView]
    ...
    def doNormalOperationalStateChecks(channel: Channel) = {
      ...
      val isIncomingFeeTooHigh = extraRoute.nonEmpty && LNParams.isFeeBreach(
        extraRoute,
        1000000000L,
        percent = 100L
      )
      if (isIncomingFeeTooHigh)
        setExtraInfo(
          text = me getString ln_info_high_fee format extraRoute.head.feeBreakdown
        )
      val hasStuckHtlcs = channel.inFlightHtlcs.exists(
        _.add.expiry <= LNParams.broadcaster.currentHeight
      )
  ...
  class NormalViewHolder(view: View) extends ChanViewHolder(view) {
    def fill(chan: NormalChannel, cs: NormalCommits): NormalViewHolder = {
      ...
      val barCanSend =
        (cs.remoteCommit.spec.toRemoteMsat / capacitySat.toLong).toInt
      ...
      paymentsInFlightText setText sumOrNothing(
        chan.inFlightHtlcs.toVector.map(_.add.amountMsat).sum.fromMsatToSat
      ).html
      canReceiveText setText denom
        .parsedWithSign(chan.estCanReceiveMsat.toTruncatedMsat)
        .html
      canSendText setText denom
        .parsedWithSign(chan.estCanSendMsat.toTruncatedMsat)
        .html
      refundableAmountText setText sumOrNothing(
        chan.refundableMsat.fromMsatToSat
      ).html
      ...
      chan.data match {
        case norm: NormalData if isOperational(chan) =>
          ...
          visibleExcept(
            gone = R.id.fundingDepth,
            R.id.closedAt,
            R.id.hostedWarningHeader
          )
        ...
        case norm: NormalData =>
          ...
          if (norm.unknownSpend.isDefined)
            setExtraInfo(resource = ln_info_unknown_spend)
          visibleExcept(
            gone = R.id.baseBar,
            R.id.overBar,
            R.id.canSend,
        ...
        case wait: WaitBroadcastRemoteData =>
          if (lostFunding) setExtraInfo(resource = ln_info_funding_lost)
          if (wait.fundingError.isDefined)
            setExtraInfo(text = wait.fundingError.get)
          visibleExcept(
            gone = R.id.baseBar,
            R.id.overBar,
            R.id.canSend,
        ...
        case cd: ClosingData =>
          ...
          visibleExcept(
            ...
            R.id.fundingDepth,
            R.id.paymentsInFlight,
            R.id.refundableAmount,
            R.id.refundFee,
            R.id.balancesDivider,
            R.id.totalPayments,
            R.id.hostedWarningHeader
          )
      ...
      view setOnClickListener onButtonTap {
        ...
        val addressHint =
          getString(ln_chan_close_address) -> closeBitcoinAddress match {
            case base \ Some(bitcoinAddress) =>
              base format humanSix(bitcoinAddress.toString)
            case base \ _ => base format getString(ln_chan_close_address_hint)
          }

        val lst \ alert = makeChoiceList(
          chan.data match {
            // Unknown spend may be our own future commit, don't allow force-closing here
            case norm: NormalData if norm.unknownSpend.isDefined =>
              normalChanActions take 1
        ...
        lst setOnItemClickListener onTap { pos =>
          def warnAndMaybeClose(channelClosureWarning: String) = {
            val bld = baseTextBuilder(channelClosureWarning.html)
              .setCustomTitle(chan.data.announce.asString.html)
            mkCheckForm(
              alert => rm(alert)(chan process ChannelManager.CMDLocalShutdown),
              none,
              bld,
              dialog_ok,
              dialog_cancel
          ...
          def warnAndMaybeCloseToAddress = closeBitcoinAddress map {
            bitcoinAddress =>
              val text = getString(ln_chan_close_confirm_address) format humanSix(
                bitcoinAddress.toString
              )
              val bld = baseTextBuilder(text.html)
                .setCustomTitle(chan.data.announce.asString.html)
              val program =
                ScriptBuilder.createOutputScript(bitcoinAddress).getProgram
              val customShutdown = CMDShutdown apply Some(
                ByteVector view program
              )
              mkCheckForm(
                alert => rm(alert)(chan process customShutdown),
                none,
                bld,
                dialog_ok,
                dialog_cancel
          ...
          rm(alert) {
            ...
            if (0 == pos)
              host.browse(s"https://smartbit.com.au/tx/${chan.fundTxId.toHex}")
            else if (1 == pos && canCoopClose && noHtlcBlock)
              warnAndMaybeClose(me getString ln_chan_close_confirm_wallet)
            else if (1 == pos && canCoopClose)
              warnAndMaybeClose(me getString ln_chan_close_inflight_details)
            else if (1 == pos)
              warnAndMaybeClose(
                channelClosureWarning = me getString ln_chan_force_details
              )
            else if (2 == pos && closeBitcoinAddress.isEmpty)
              host toast ln_chan_close_address_hint
            else if (2 == pos && canCoopClose && noHtlcBlock)
              warnAndMaybeCloseToAddress
  ...
  class HostedViewHolder(view: View) extends ChanViewHolder(view) {
    def fill(chan: HostedChannel, hc: HostedCommits): HostedViewHolder = {
      val capacitySat =
        hc.lastCrossSignedState.initHostedChannel.channelCapacityMsat.fromMsatToSat
      ...
      hc.localError -> hc.remoteError match {
        case _ \ Some(err) =>
          setExtraInfo(s"Remote: ${ChanErrorCodes translateTag err}")
        case Some(err) \ _ =>
          setExtraInfo(s"Local: ${ChanErrorCodes translateTag err}")
      ...
      view setOnClickListener onButtonTap {
        ...
        lst setOnItemClickListener onTap { pos =>
          ...
          def removeChan = {
            ...
            ConnectionManager.workers
              .get(hc.announce.nodeId)
          ...
          def shareStateAsFile = {
            ...
            val text = getString(ln_hosted_state_wrap).format(
              hc.channelId.toHex,
              data,
              preimages
            )
            val savedFile = new File(
              snapshotFilePath,
              s"Hosted channel state ${new Date}.txt"
            )
          ...
          rm(alert) {
            if (1 == pos) shareStateAsFile
              ...
              host.browse(s"https://lightning-wallet.com/hosted-channels")
            else if (2 == pos && chan.inFlightHtlcs.nonEmpty)
              warnAndMaybeRemove(me getString ln_hosted_remove_inflight_details)
            else if (2 == pos && chan.estCanSendMsat.fromMsatToSat > LNParams.dust)
              warnAndMaybeRemove(
                me getString ln_hosted_remove_non_empty_details
              )
  ...
  override def onOptionsItemSelected(m: MenuItem) = runAnd(true) {
    if (m.getItemId == R.id.actionAddNodeId)
      me share LNParams.nodePublicKey.toString
  ...
  def INIT(s: Bundle) =
    if (app.isAlive) {
      ...
      getSupportActionBar setSubtitle app.plur1OrZero(
        barStatus,
        displayedChans.size
      )
  ...
  def drainHostedChan = {
    ...
    val hosted1 = hosted.filter(
      hostedChan =>
        isOperational(hostedChan) && hostedChan.estCanSendMsat.fromMsatToSat > LNParams.dust
    )
    val normal1 = normal.filter(
      chan =>
        isOperational(chan) && chan.estCanReceiveMsat.fromMsatToSat > LNParams.dust && channelAndHop(
          chan
        ).nonEmpty
    )
    ...
    if (hosted1.isEmpty) me toast err_ln_drain_no_hosted
      ...
      val normalExtraRoutes = normal1 flatMap channelAndHop collect {
        case _ \ normalExtraHop => normalExtraHop
      }
      val largestCanSendMinusFeesMsat = largestCanSendMsat - LNParams
        .maxAcceptableFee(largestCanSendMsat, hops = 3)
      val transferSum = math
        .min(
          normal1.map(_.estCanReceiveMsat).sorted.head,
          largestCanSendMinusFeesMsat
        )
        .millisatoshi
      val rd = PaymentInfoWrap.recordRoutingDataWithPr(
        normalExtraRoutes,
        transferSum,
        preimage,
        REBALANCING
  ...
  def stateStatusColor(c: Channel) = (c.data, c.state) match {
    ...
    case (_: NormalData, OPEN) if isOperational(c) =>
      me getString ln_info_status_open
    case (_: NormalData, _) if !isOperational(c) =>
      me getString ln_info_status_shutdown
    case (_: HasNormalCommits, WAIT_FUNDING_DONE) =>
      me getString ln_info_status_opening
    case (_: HasNormalCommits, NEGOTIATIONS) =>
      me getString ln_info_status_negotiations
